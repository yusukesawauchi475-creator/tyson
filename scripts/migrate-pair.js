...
function generateRandomSlug(length = SLUG_LEN) {
  let s = '';
  for (let i = 0; i < length; i++) {
    s += SLUG_CHARS.charAt(Math.floor(Math.random() * SLUG_CHARS.length));
  }
  return s;
}

async function findUniqueSlug(maxAttempts = 20) {
  for (let i = 0; i < maxAttempts; i++) {
    const candidate = generateRandomSlug();
    const snap = await db.collection(SLUGS_COLLECTION).doc(candidate).get();
    if (!snap.exists) return candidate;
  }
  throw new Error(`Slug collision retry exceeded after ${maxAttempts} attempts`);
}
...
async function main() {
  ...
  // 2. 衝突しない新 slug 生成
  const newSlug = await findUniqueSlug();
