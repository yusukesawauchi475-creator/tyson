...
export default function PairDailyPage({ lang = 'ja', onChangeRole, role = 'child' }) {
  ...
  useEffect(() => {
    const d = new Date()
    setToday(d.toLocaleDateString(lang === 'en' ? 'en-US' : lang === 'es' ? 'en-US' : 'ja-JP', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
      weekday: 'short',
      ...
      <main className="page-content page" style={{ flex: 1, maxWidth: 480, margin: '0 auto', width: '100%', paddingTop: 8 }}>
        ...
        <section style={{ width: '100%', background: '#f5d8e0', borderRadius: 20, padding: 16, boxShadow: '0 4px 0 0 #c98fa0', overflow: 'hidden', fontFamily: 'Nunito, sans-serif' }}>
          ...
          <p style={{ textAlign: 'center', fontSize: 12, color: '#6b2a3a', margin: '0 0 8px', fontWeight: 600 }}>
            ▶ {lang === 'en' ? 'Recording as Child' : lang === 'es' ? 'Recording as Child' : '子として録音します'} 👦👧
          </p>
          <div style={{ position: 'relative', width: '100%' }}>
        ...
        <section style={{ width: '100%', background: '#d4bfff', borderRadius: 20, padding: 16, boxShadow: '0 4px 0 0 #8b6bd4', overflow: 'hidden', fontFamily: 'Nunito, sans-serif' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 15, fontWeight: 800, color: '#3a1a7a' }}>
              {lang === 'en' ? 'Send photos' : lang === 'es' ? 'Send photos' : '写真を送る'} · {photos.filter((p) => p.role === ROLE_CHILD).length}/3
            </span>
          </div>
