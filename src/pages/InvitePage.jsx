...
export default function InvitePage() {
  ...
  const handleShare = async () => {
    if (!pairId) {
      setToastMsg(lang === 'en' ? 'Pair ID not found' : lang === 'es' ? 'ID de pareja no encontrado' : 'ペアIDが見つかりません')
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, gap: 20, fontFamily: 'var(--font-sans)', background: '#FFF8FF' }}>
      <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0, color: '#333' }}>
        👋 {lang === 'en' ? 'Invite' : lang === 'es' ? 'Invitar' : '招待'}
      </h1>
      <button
        type="button"
        onClick={handleShare}
      ...
      >
        {lang === 'en' ? 'Start sharing' : lang === 'es' ? 'Comenzar a compartir' : '共有を開始'}
      </button>
      <Link to={`/pair/${slug}`} style={{ fontSize: 14, color: '#7050C0', textDecoration: 'none' }}>
        ← {lang === 'en' ? 'Back to Home' : lang === 'es' ? 'Volver al inicio' : 'ホームへ戻る'}
      </Link>
