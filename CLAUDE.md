...
| `/pair/:slug` | PairWorld → RootRoute | ペア世界ラッパー (role振り分け) |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けランディング (animation付き) |
| `src/pages/WelcomePage.jsx` | ウェルカムページ (⚠️ lang prop 未受取・日本語固定) |
| `src/pages/InvitePage.jsx` | 招待ページ (⚠️ lang='ja' ハードコード) |

### フロントエンド (Components)
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug ラッパー (slug → pairId 解決, Outlet) |
...
| `src/components/InviteModal.jsx` | 招待モーダル (Messenger/LINE/Instagram等) |
| `src/components/DemoModal.jsx` | デモCTAモーダル |
| `src/components/UploadErrorModal.jsx` | アップロードエラーモーダル |
| `src/components/FamilyInsightCard.jsx` | 家族インサイトAIカード (⚠️ lang無視キャッシュ) |
| `src/components/Visualizer.jsx` | 録音/再生時 音声ビジュアライザー |
| `src/components/LanguageSwitch.jsx` | 言語切替UI (ja/en/es) |
| `src/components/RoleBadge.jsx` | 親/子ロールバッジ |
| `src/components/AdminAuth.jsx` | ⚠️ dead code (どこにもimportされていない) |
| `src/components/OneYearAgoBanner.jsx` | ⚠️ dead code (どこにもimportされていない) |
...
| `api/daily-theme.js` | AI話題生成 (⚠️ 認証なし・pairIdバリデーションなし) |
| `api/family-insight.js` | 家族インサイトAI生成 (⚠️ !pairId時に200返却) |
...
| `api/lib/pair-access.js` | ペアアクセス検証 (isPairAllowed, isTysonOnlyBlocked 等) |
