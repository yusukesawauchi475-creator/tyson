...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → LandingPage | last_slug あり→ /pair/:slug, なし→ランディング |
...
| `/pair/:slug/` | RootRoute → RoleSelectPage/HomePage/PairDailyPage | role振り分け |
...
| `src/pages/HomePage.jsx` | 親のホーム画面 (緑/ピンク/紫の3カード) ⚠️ 無効API呼び出し残存(L199,212,231) |
| `src/pages/PairDailyPage.jsx` | 子のホーム画面 ⚠️ 無効API呼び出し残存(L136,603,635,680) |
...
| `src/pages/WelcomePage.jsx` | ウェルカムページ |
| `src/pages/InvitePage.jsx` | 招待ページ (/pair/:slug/invite) |

### コンポーネント (src/components/)
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug Outlet — slug→pairId 解決・PairContext 供給 |
...
| `src/components/DemoModal.jsx` | デモ案内モーダル |
| `src/components/UploadErrorModal.jsx` | アップロードエラーモーダル |
| `src/components/AdminAuth.jsx` | 管理画面認証UI |
...
| `src/lib/pairMembership.js` | ペアメンバーシップ確認 |
...
| `src/lib/useAudioLevel.js` | マイク音量フック |
...
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history ⚠️ merge:true 残存(L833,L1110) |
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST ⚠️ merge:true 残存(L553) |
| `api/album.js` | アルバム全日写真取得 ⚠️ initFirebaseAdmin順序問題(L128) |
| `api/invite.js` | ペア発行(create-numbered), スラグ解決(resolve) |
| `api/streak.js` | 連続記録ストリーク ⚠️ PAIR-DEMOTEST write block 欠落 |
| `api/daily-theme.js` | AI話題生成 |
| `api/admin-reset.js` | 管理リセット ⚠️ PAIR-DEMOTEST write block 欠落 |
| `api/admin-restore.js` | 管理復元 ⚠️ PAIR-DEMOTEST write block 欠落 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/family-insight.js` | AI家族インサイト生成 |
| `api/journal-analysis.js` | 日記分析 ⚠️ CRITICAL: Firebase Auth なし・任意pairIdでFirestore読み書き可能 |
| `api/lib/` | API共通ライブラリ (pair-access.js 等) |
| `api/_disabled/` | 無効化済みAPI (analyze.js, analysis-comment.js 等) |
