...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → LandingPage | 最終アクセスslug有→/pair/:slug へリダイレクト, 無→ランディング |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | ウェルカムページ |
| `/pair/:slug` | PairWorld (Outlet) | Pair ワールド (slug → pairId 解決) |
| `/pair/:slug/` | RootRoute → HomePage or PairDailyPage | role有→ホーム, 無→RoleSelectPage |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | ルーティング, RootOrLanding, RootRoute (role振り分け), lang解決 |
...
| `src/pages/AdminPage.jsx` | 管理画面 (ペア発行, ダッシュボード) ※日本語のみ |
...
| `src/pages/WelcomePage.jsx` | ウェルカムページ ⚠️ lang prop 未使用 (日本語固定バグあり) |
| `src/pages/InvitePage.jsx` | 招待ページ ⚠️ lang ハードコード 'ja' 固定バグあり |

### フロントエンド — Components
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug Outlet wrapper (slug→pairId解決, lang/pairId context提供) |
...
| `src/components/FamilyInsightCard.jsx` | AIファミリーインサイトカード ⚠️ lang未考慮キャッシュキーバグあり |
| `src/components/Visualizer.jsx` | 録音/再生中の波形ビジュアライザー (button overlay) |
...
| `src/components/OneYearAgoBanner.jsx` | ⚠️ dead code — どこからも import されていない |
...
| `src/lib/i18n.js` | 日英西 (ja/en/es) 翻訳 |
| `src/lib/voiceRole.js` | getEffectiveRole (role correction 適用) |
| `src/lib/unreadState.js` | 未読状態管理 (Firestore + ローカル) |
...
| `src/lib/inviteShare.js` | 招待シェアロジック |
| `src/lib/dateFormat.js` | 日付フォーマットユーティリティ (NY時間基準) |
| `src/lib/uiCopy.js` | UIコピーテキスト定数 |
| `src/lib/demoPhotos.js` | デモ用写真データ ⚠️ 日付ラベル日本語固定 |
| `src/lib/tysonThemes.js` | テーマカラー定数 |
| `src/lib/useAudioLevel.js` | 録音音量レベル取得 hook |
| `src/lib/indexedDB.js` | IndexedDB オフラインキャッシュ |
| `src/lib/fcm.js` | Firebase Cloud Messaging (プッシュ通知) |
| `src/lib/shareTargets.js` | SNSシェアターゲット |
...
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen/correct) + voice-history ⚠️ merge:true 残存 L816,L1076 |
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST ⚠️ merge:true 残存 L536 |
| `api/album.js` | アルバム全日写真取得 |
| `api/invite.js` | ペア発行(create-numbered), スラグ解決(resolve) |
| `api/streak.js` | 連続記録ストリーク ⚠️ PAIR-DEMOTEST書き込みブロック欠落 |
| `api/daily-theme.js` | AI話題生成 ⚠️ verifyIdToken 未実装 (Critical) |
| `api/family-insight.js` | AIファミリーインサイト生成 ⚠️ !pairId時HTTP 200返却バグ |
| `api/journal-analysis.js` | 日記AI分析 ⚠️ merge:true 残存 L120,L176 |
| `api/admin-reset.js` | 管理リセット ⚠️ merge:true 残存 L151 |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | isPairAllowed() (pair membership 検証), DEMO_PAIR_IDS bypass |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase service account パース |
| `api/_disabled/` | 無効化済みAPI群 (analyze, analysis-comment 等) ⚠️ フロントエンドから参照残存 |
...
pair_members/{pairId}/members/{uid} → { role, joinedAt, ... }  ← pair membership 管理
pair_media/{pairId}/days/{dateKey}  → { parent: { audioPath[], latestAudioPath, uploadedBy, mimeType, deviceHint, roleAtUpload, seenAt, ... }, child: {...} }
