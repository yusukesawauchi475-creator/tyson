...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding | localStorage に slug あり → `/pair/:slug` にリダイレクト、なし → LandingPage |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けランディング |
| `/welcome` | WelcomePage | 新規ペア作成後オンボーディング |
| `/pair/:slug` | PairWorld (outlet) | slug → pairId 解決 + membership claim |
| `/pair/:slug/` (index) | RootRoute → HomePage / PairDailyPage / RoleSelectPage | role に応じて振り分け |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待リンク共有 |

## ペアの仕組み
- 公開URL: `humfamily.com/pair/{8文字スラグ}` (例: /pair/ulf1q6ab)
- Vercel rewrites: `/(*)` → `/index.html`（SPA、クライアントサイドルーティング）
- PairWorld コンポーネントが `pair_numbers/{slug}` を Firestore から直接読み取り pairId を解決
- 解決成功後 `ensureAuthAndMembership()` で pair_users/{pairId}/members/{uid} を claim
...
| `AdminPage.jsx` | 管理画面 (ペア発行, ダッシュボード) |
| `RoleSelectPage.jsx` | 親/子の役割選択 |
| `LandingPage.jsx` | 初回訪問ランディング |
| `DemoPage.jsx` | デモ体験 |
| `FacilitiesPage.jsx` | 介護施設向けランディング (デモアニメーション付き) |
| `WelcomePage.jsx` | 新規ペア作成後オンボーディング (招待リンク共有) |
| `InvitePage.jsx` | 招待リンク共有ページ (`/pair/:slug/invite`) |

### ルーティング・コンテキスト (src/)
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter, Routes定義, RootOrLanding/RootRoute/AppRoutes |
| `src/components/PairWorld.jsx` | `/pair/:slug` outlet — slug→pairId解決 + membership claim + Outlet context |
...
| `AlbumCalendar.jsx` | アルバムカレンダービュー |
| `RoleBadge.jsx` | 親/子ロールバッジ (タップでロール切替) |
| `InviteModal.jsx` | 招待モーダル |
| `DemoModal.jsx` | デモ CTA モーダル |
| `FamilyInsightCard.jsx` | AI生成ファミリーインサイトカード |
| `UploadErrorModal.jsx` | アップロードエラーモーダル |
| `Visualizer.jsx` | 録音中の音声波形ビジュアライザー |
| `LanguageSwitch.jsx` | 言語切替 (ja/en/es) |
| `OneYearAgoBanner.jsx` | 1年前の記録バナー |
| `AdminAuth.jsx` | 管理画面認証コンポーネント |

### ライブラリ (src/lib/)
| ファイル | 役割 |
|---------|------|
| `pairDaily.js` | getUserRole, setUserRole, uploadAudio, fetchAudioForPlayback, getStreak 等 |
| `pairMembership.js` | ensureAuthAndMembership (slug→pairId + membership claim) |
| `journal.js` | 写真アップロード, fetchTodayJournalMeta, fetchAlbum |
| `firebase.js` | Firebase初期化, getIdTokenForApi (匿名認証), auth self-heal |
| `i18n.js` | 日英西翻訳 (ja/en/es) |
| `pairSlug.js` | findUniqueSlug (Crockford Base32, 8文字) |
| `voiceRole.js` | getEffectiveRole (correctedRole/roleAtUpload 解決) |
| `dateFormat.js` | 日付フォーマットユーティリティ |
| `unreadState.js` / `unreadStateCore.js` | 未読バッジ状態管理 |
| `listenedTracking.js` | 再生済み追跡 (IndexedDB) |
| `indexedDB.js` | IndexedDB ヘルパー |
| `fcm.js` | FCM プッシュ通知登録 (registerForPush — 現在どこからも呼ばれていない) |
| `inviteShare.js` | 招待リンク共有ロジック |
| `shareTargets.js` | SNS共有先 (LINE/Twitter/etc) |
| `tysonThemes.js` | ペアテーマカラー |
| `uiCopy.js` | UIコピーテキスト定数 |
| `holidayBanner.js` | 祝日バナーデータ |
| `demoPhotos.js` | デモ用静的写真データ |
| `deployHealthCheck.js` | デプロイ健全性チェック |
| `useAudioLevel.js` | 録音音量レベル React hook |
| `invite.js` | 招待 API クライアント |

### API (Vercel Serverless, api/)
| ファイル | 役割 |
|---------|------|
| `pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history + voice-month + role-history-record |
| `journal.js` | 写真 GET(今日→最新日フォールバック)/POST |
| `album.js` | アルバム全日写真取得 |
| `invite.js` | ペア発行(create-numbered/create-welcome), スラグ解決(resolve), membership claim |
| `streak.js` | 連続記録ストリーク GET(計算)/POST(保存) |
| `daily-theme.js` | AI話題生成 (OpenAI) |
| `family-insight.js` | AIファミリーインサイト生成 (OpenAI) |
| `journal-analysis.js` | 日記画像OCR+AI分析 (OpenAI Vision, admin専用) |
| `admin-reset.js` | 管理リセット (journal日次データクリア) |
| `admin-restore.js` | 管理復元 (スナップショットから復元) |
| `admin-pairs.js` | ペアダッシュボード |
| `lib/pair-access.js` | isPairAllowed / isTysonOnlyBlocked (pair membership 認可ヘルパー) |
| `lib/parseFirebaseServiceAccount.js` | Firebase SA JSON パース |
| `_disabled/` | 無効化済み関数 (analyze.js / analysis-comment.js 等) |

### 設定
| ファイル | 役割 |
|---------|------|
| `vercel.json` | SPA リライト (`/*` → `/index.html`), キャッシュヘッダー |
| `firebase.json` | Firestore/Storage ルール参照 |
| `firestore.rules` | Firestore セキュリティルール (auth + pair membership 必須) |
| `storage.rules` | Storage セキュリティルール (client SDK write 全禁止, API経由のみ) |
...
- BrowserRouter形式（/pair/:slug/album 等）が全リンクで使われてるか確認（旧HashRouter `/#/` 形式が残っていないか）
