...
| `/welcome` | WelcomePage | CTA経由の新pair自動発行 |
| `/pair/:slug` | PairWorld (Outlet) | pair世界のルート (slug解決) |
| `/pair/:slug` (index) | RootRoute | 親/子ホーム画面 (role振り分け) |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待URL発行・共有 |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | ルーティング (BrowserRouter), PairWorld Outlet, AppRoutes |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けランディング (Formspree問い合わせフォーム付き) |
| `src/pages/WelcomePage.jsx` | CTA経由訪問者への新pair自動発行・招待URL表示 |
| `src/pages/InvitePage.jsx` | pair内招待URL発行・共有 (useOutletContext) |
...
| `src/components/AdminAuth.jsx` | **dead code** (src/ 内に import なし) |
| `src/lib/pairDaily.js` | getDateKeyNY, getUserRole, markSeen, uploadAudio, fetchAudio 等 |
| `src/lib/pairMembership.js` | ensureAuthAndMembership (auth + pair membership claim) |
...
| `api/journal-analysis.js` | 写真OCR (OpenAI gpt-4o-mini) ※既知バグあり: batch mode で再OCR時に stale snapshot からsibling overwrite リスク |
| `api/lib/pair-access.js` | isTysonOnlyBlocked, isPairAllowed (共通ヘルパー) |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase SA パース |
| `api/_disabled/` | 無効化済みAPI (analyze.js, analysis-comment.js 等) — Vercel未デプロイ |
...
- BrowserRouter形式（/pair/:slug/...）が全リンクで使われてるか確認（`/#/` は旧形式）
