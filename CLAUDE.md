...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → RootRoute | 未ログイン→ランディング |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/welcome` | WelcomePage | ペア作成後の招待画面 (⚠️ lang prop 未接続) |
| `/pair/:slug` | PairWorld → RootRoute | pairId解決・role振り分け (Outlet) |
| `/pair/:slug/album` | PairWorld → AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | PairWorld → InvitePage | 招待ページ (⚠️ lang='ja' 固定) |

## ペアの仕組み
- 公開URL: `humfamily.com/pair/{8文字 Crockford Base32 slug}` (例: /pair/ulf1q62a)
- PairWorld コンポーネントがクライアントサイドで Firestore `pair_numbers/{slug}` → `pairId` を解決
...
| `src/pages/WelcomePage.jsx` | ペア作成後招待画面 (**⚠️ lang prop なし・全文日本語固定**) |
| `src/pages/InvitePage.jsx` | 招待リンク共有 (**⚠️ lang='ja' ハードコード**) |

### フロントエンド (Components)
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug の Outlet wrapper、slug→pairId 解決 |
...
| `src/components/DemoModal.jsx` | デモペア誘導モーダル |
| `src/components/InviteModal.jsx` | 招待リンクコピーモーダル (**⚠️ lang prop なし・日本語固定**) |
| `src/components/LanguageSwitch.jsx` | JP/EN 切り替え |
...
| `src/components/Visualizer.jsx` | 録音・再生中 波形 visualizer |
| `src/components/AlbumCalendar.jsx` | アルバムカレンダービュー |
| `src/components/AdminAuth.jsx` | **⚠️ dead code（どこからもimport不要）** |
...
| `src/lib/shareTargets.js` | 共有先定義 (**⚠️ label日本語固定**) |
| `src/lib/fcm.js` | FCMプッシュ通知 |
...
| `src/lib/useAudioLevel.js` | 音声レベル取得 React hook |
...
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history (**⚠️ pairId fallback `\|\| 'demo'`**) |
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST (**⚠️ pairId fallback `\|\| 'demo'`**) |
...
| `api/daily-theme.js` | AI話題生成 (**🔴 Critical: verifyIdToken なし・認証不要**) |
| `api/family-insight.js` | 家族インサイト生成 (**⚠️ UTC日付使用・pairId空で200返却**) |
| `api/journal-analysis.js` | 写真AIコメント生成 |
| `api/admin-reset.js` | 管理リセット (**⚠️ pairId fallback `\|\| 'demo'`**) |
| `api/admin-restore.js` | 管理復元 (**⚠️ pairId fallback `\|\| 'demo'`**) |
...
- BrowserRouter形式（/pair/:slug/album 等）が全リンクで使われてるか確認（HashRouter `/#/` は旧形式）
