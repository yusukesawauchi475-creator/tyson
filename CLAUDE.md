...
| `/welcome` | WelcomePage | DEMO CTA 経由 pair 自動発行 |
| `/pair/:slug` | PairWorld (Outlet) | pair コンテキスト確立 |
| `/pair/:slug/` (index) | RootRoute | 親/子ホーム (role振り分け) |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | ルーティング, PairWorld Outlet, RootRoute (role振り分け) |
...
| `src/pages/RoleSelectPage.jsx` | 親/子の役割選択 (role_history記録) |
| `src/pages/LandingPage.jsx` | 初回訪問ランディング |
| `src/pages/DemoPage.jsx` | デモ体験 |
| `src/pages/FacilitiesPage.jsx` | 介護施設向けランディング (デモアニメーション) |
| `src/pages/WelcomePage.jsx` | DEMO CTA 経由で来訪したユーザーへの pair 自動発行 |
| `src/pages/InvitePage.jsx` | ペア招待ページ (LINE/コピー共有) |
| `src/components/PairWorld.jsx` | pair コンテキスト確立 Outlet (slug→pairId解決, membership確認) |
...
| `src/components/DemoModal.jsx` | デモ用モーダル (write block CTA) |
...
| `src/lib/pairMembership.js` | pair membership 確認 (フロントエンド側) |
| `src/lib/fcm.js` | FCM push通知登録 (registerForPush — 現在未呼び出し) |
| `src/lib/indexedDB.js` | IndexedDB キャッシュ管理 |
| `src/lib/pairSlug.js` | slug ↔ pairId 解決ヘルパー |
| `src/lib/inviteShare.js` | 招待URL生成・共有ヘルパー |
| `src/lib/voiceRole.js` | 音声 role (parent/child) 管理 |
| `src/lib/listenedTracking.js` | 再生済みトラッキング |
| `src/lib/unreadState.js` | 未読状態管理 |
| `src/lib/unreadStateCore.js` | 未読状態コアロジック |
| `src/lib/useAudioLevel.js` | 音声レベル取得カスタムフック |
| `src/lib/shareTargets.js` | 共有ターゲット定義 |
| `src/lib/tysonThemes.js` | テーマ定義 |
| `src/lib/uiCopy.js` | UIコピー (ラベル等) 定義 |
...
| `src/lib/invite.js` | 招待リンクコピーヘルパー |
| `src/index.css` | グローバルCSS (.page, .bottom-nav 等) |

### API (Vercel Serverless)
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history + role_history |
...
| `api/daily-theme.js` | AI話題生成 (Firebase Auth + isPairAllowed 必須) |
| `api/family-insight.js` | 家族インサイト生成 (7日分ルックバック) |
| `api/journal-analysis.js` | ジャーナル分析 (⚠️ Firebase Auth なし — Critical未修正) |
| `api/admin-reset.js` | 管理リセット (Firebase Auth + isPairAllowed) |
| `api/admin-restore.js` | 管理復元 (Firebase Auth + isPairAllowed) |
| `api/admin-pairs.js` | ペアダッシュボード (X-Admin-Password認証) |
...
| `api/_disabled/` | 無効化済みエンドポイント (analyze.js, analysis-comment.js) |
...
- **PAIR-DEMOTEST**: デモ用。`READ_ONLY_PAIR_IDS`（pair-media.js のみ）で書き込みブロック。admin-reset/admin-restore/streak では未ブロック（既知 bug #107）
...
- BrowserRouter形式（/path）が全リンクで使われてるか確認（HashRouter廃止）
