...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding | hum_last_slug あり→/pair/:slug リダイレクト, 無→LandingPage |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けページ（※既知バグ: lang prop 未受け取り） |
| `/welcome` | WelcomePage | DEMO CTA 経由の新 pair 発行（※既知バグ: lang prop 未受け取り） |
| `/pair/:slug` | PairWorld (Outlet) | slug→pairId 解決, 認証, メンバーシップ確認 |
| `/pair/:slug/` | RootRoute | role判定→HomePage(親) / PairDailyPage(子) / RoleSelectPage |
| `/pair/:slug/album` | AlbumPage | 写真タブ/声タブ, カレンダービュー |
| `/pair/:slug/invite` | InvitePage | 招待ページ（※既知バグ: lang='ja'ハードコード） |

## ペアの仕組み
- 公開URL: `humfamily.com/pair/{6文字スラグ}` (例: /pair/ulf1q6)
- Vercel は SPA rewrite (`/pair/:slug` → `index.html`)。API redirect なし。
- PairWorld コンポーネントが client-side で Firestore `pair_numbers/{slug}` を解決
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter ルーティング定義, RootOrLanding, RootRoute, AppRoutes |
...
| `src/pages/WelcomePage.jsx` | DEMO CTA → 新 pair 自動発行 → 招待 URL 表示 |
| `src/pages/InvitePage.jsx` | pair 招待ページ (lang='ja' ハードコード中) |
| `src/pages/FacilitiesPage.jsx` | 介護施設向け紹介ページ |

### コンポーネント (src/components/)
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | slug 解決 + 認証 + メンバーシップ確認 + Outlet提供 |
...
| `src/components/FamilyInsightCard.jsx` | 家族の振り返りコメント表示 |
| `src/components/DemoModal.jsx` | DEMO CTA モーダル |
| `src/components/InviteModal.jsx` | 招待リンクシェアモーダル |
| `src/components/Visualizer.jsx` | 録音/再生時の音声波形ビジュアライザー |
...
| `src/lib/pairMembership.js` | ensureAuthAndMembership (slug→pairId解決+認証+メンバー確認) |
| `src/lib/firebase.js` | Firebase初期化, getIdTokenForApi (匿名認証) |
| `src/lib/i18n.js` | 日英翻訳 |
| `src/lib/unreadState.js` | 未読バッジ状態管理 |
| `src/lib/unreadStateCore.js` | 未読判定コアロジック |
| `src/lib/demoPhotos.js` | PAIR-DEMOTEST 用静的デモ写真データ |
...
| `src/lib/inviteShare.js` | 招待シェア (LINE/その他) |
| `src/lib/shareTargets.js` | シェアターゲット定義 |
| `src/lib/tysonThemes.js` | テーマ/カラー定義 |
| `src/lib/uiCopy.js` | UIコピー文言定義 |
| `src/lib/listenedTracking.js` | 再生済みトラッキング |
| `src/lib/useAudioLevel.js` | 録音中の音量レベル取得 (Visualizer用) |
| `src/lib/indexedDB.js` | IndexedDB操作ユーティリティ |
| `src/lib/pairSlug.js` | スラグ生成・バリデーション |
...
| `api/invite.js` | ペア発行(create-numbered/create-welcome), スラグ解決(resolve), メンバー追加 |
| `api/streak.js` | 連続記録ストリーク GET/POST |
| `api/daily-theme.js` | AI話題生成 (OpenAI, pairId任意でパーソナライズ) |
| `api/family-insight.js` | 家族インサイトコメント生成 (※既知バグ: pairId未指定時に200を返す) |
| `api/journal-analysis.js` | 写真OCR解析 (※Critical既知バグ: Firebase Auth なし) |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/_disabled/` | 無効化済みAPI (upload.js, analyze.js) |
...
pair_users/{pairId}/members/{uid}                     → { role, claimedAt }
pair_users/{pairId}/parentDevices/{deviceId}          → { fcmToken, ... }  ← client write 可
pair_members/{pairId}/...                             → 追加メンバー管理 (Admin SDK only)
pairs/{pairId}/meta/streak                            → { count, lastDateKey, firstDateKey }
role_history/{pairId}/...                             → role切替の immutable 履歴 (Admin SDK only)
```
...
- BrowserRouter形式（/pair/:slug/path）が全リンクで使われてるか確認（`/#/`形式は廃止）
