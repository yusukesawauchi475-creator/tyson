...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding | slug stored → /pair/:slug リダイレクト, 無 → LandingPage |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向け B2B landing page (Formspree contact form) |
| `/demo` | DemoPage | デモ体験 |
| `/welcome` | WelcomePage | ウェルカム画面 ⚠️ lang prop 未接続バグあり |
| `/admin` | AdminPage | 管理画面 (ペア発行, ダッシュボード) |
| `/pair/:slug` | PairWorld | pair コンテキスト解決 (Firestore slug→pairId) |
| `/pair/:slug` (index) | RootRoute | role なし→RoleSelectPage, parent→HomePage, child→PairDailyPage |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待ページ ⚠️ lang='ja' hardcode バグあり |

## ペアの仕組み
- 公開URL: `humfamily.com/pair/{6文字スラグ}` (例: /pair/ulf1q6)
- Vercel SPA rewrite → `index.html` → BrowserRouter → `PairWorld` component
- `PairWorld` が client-side で Firestore `pair_numbers/{slug}` を読み `pairId` を解決
- Firestore: `pair_numbers/{slug}` → `pairId` (例: PAIR-H58HTP)
- 内部ID `PAIR-XXXXXX` は URL に露出しない
- pairId が null の場合はエラー表示のみ — fallback 絶対禁止
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter ルーティング, lang 解決 (URL ?lang=/ localStorage), AppRoutes |
...
| `src/pages/LandingPage.jsx` | 初回訪問ランディング (Facility CTA → /facilities) |
| `src/pages/DemoPage.jsx` | デモ体験 |
| `src/pages/FacilitiesPage.jsx` | 介護施設 B2B landing (Formspree, ⚠️ animation 内テキスト日本語 hardcode, ⚠️ VITE_FORMSPREE_ENDPOINT 要設定) |
| `src/pages/InvitePage.jsx` | 招待ページ ⚠️ `const lang = 'ja'` hardcode バグ (en/es dead code) |
| `src/pages/WelcomePage.jsx` | ウェルカム画面 ⚠️ lang prop 受け取り未実装 |

### Components
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | pair コンテキスト解決 (slug→pairId), Outlet へ pairId/lang 注入 |
...
| `src/components/InviteModal.jsx` | 招待モーダル (messenger 振り分け) |
| `src/components/DemoModal.jsx` | デモ促進モーダル |
| `src/components/LanguageSwitch.jsx` | 言語切替 (ja/en/es, URL ?lang= + localStorage) |
...
| `src/components/OneYearAgoBanner.jsx` | ⚠️ dead code + UTC日付計算バグあり |
| `src/components/FamilyInsightCard.jsx` | ⚠️ dead code + lang 無視キャッシュキーバグあり |
...
| `src/lib/i18n.js` | ja/en/es 翻訳 (t(lang, key) 関数) |
| `src/lib/listenedTracking.js` | 再生済みトラッキング (⚠️ orphaned export 2件あり) |
| `src/lib/inviteShare.js` | 招待シェア (messenger/LINE/WhatsApp 振り分け) |
| `src/lib/invite.js` | 招待URL生成 |
| `src/lib/dateFormat.js` | NY時間ベース日付フォーマット |
| `src/lib/demoPhotos.js` | デモ写真データ (⚠️ date labels 日本語 hardcode) |
| `src/lib/holidayBanner.js` | 国別記念日 countdown ロジック |
| `src/lib/fcm.js` | Firebase Cloud Messaging |
| `src/lib/indexedDB.js` | IndexedDB キャッシュ |
| `src/lib/pairSlug.js` | slug 生成 (Crockford Base32) |
| `src/lib/shareTargets.js` | SNS シェアターゲット定義 |
| `src/lib/tysonThemes.js` | UI テーマ定義 |
...
| `src/index.css` | グローバルCSS (.page, .bottom-nav, facility-animation 等) |

### API (Vercel Serverless)
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history ⚠️ L683 pairId='demo' fallback, L811/L1071 merge:true |
| `api/journal.js` | 写真 GET/POST ⚠️ L402 pairId='demo' fallback |
| `api/album.js` | アルバム全日写真取得 |
| `api/invite.js` | ペア発行(create-numbered), スラグ解決(resolve) |
| `api/streak.js` | 連続記録ストリーク ⚠️ POST が PAIR-DEMOTEST 書き込みブロックなし |
| `api/daily-theme.js` | AI話題生成 ⚠️ verifyIdToken なし (Critical) |
| `api/family-insight.js` | 家族 insight 生成 ⚠️ UTC日付計算, !pairId 時 HTTP200 |
| `api/journal-analysis.js` | ジャーナル分析 |
| `api/admin-reset.js` | 管理リセット ⚠️ L110 pairId='demo' fallback |
| `api/admin-restore.js` | 管理復元 ⚠️ L109 pairId='demo' fallback |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | pairId アクセス制御 ⚠️ TYSON_ZH90_ALLOWED_UIDS hardcode |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase service account パース |
| `api/_disabled/` | 無効化済みAPI (analyze, analysis-comment) ⚠️ frontend からまだ呼ばれている |

### 設定
| ファイル | 役割 |
|---------|------|
| `vercel.json` | SPA rewrite (BrowserRouter 対応 `/(.*) → index.html`), キャッシュヘッダー |
| `firebase.json` | Firestore/Storage ルール参照 |
| `firestore.rules` | Firestore セキュリティルール (auth必須) |
| `storage.rules` | Storage セキュリティルール (auth必須) |
| `firestore.indexes.json` | ⚠️ 現在空 (daily-theme.js の orderBy() インデックス未定義) |
