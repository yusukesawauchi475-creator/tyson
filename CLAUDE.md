...
## ルーティング (BrowserRouter + ネストルーティング)
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding | hum_last_slug あり→`/pair/:slug`へ, 無し→LandingPage |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けランディング |
| `/welcome` | WelcomePage | DEMO CTA 経由の新 pair 発行ページ |
| `/pair/:slug` | PairWorld (layout) | slug→pairId 解決、pair isolation wrapper |
| `/pair/:slug/` | RootRoute → RoleSelectPage \| HomePage \| PairDailyPage | role 振り分け |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待リンク送信 |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter, ネストルーティング, RootOrLanding, RootRoute, AppRoutes |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けランディング (screenshot animation, contact form) |
| `src/pages/InvitePage.jsx` | 招待リンク送信 (/pair/:slug/invite) |
| `src/pages/WelcomePage.jsx` | DEMO CTA 経由の新 pair 自動発行ページ |
| `src/components/PairWorld.jsx` | `/pair/:slug` layout: slug→pairId 解決, pair membership claim, Outlet |
...
| `src/components/FamilyInsightCard.jsx` | 週次家族インサイト (AI生成コメント) |
...
| `src/components/InviteModal.jsx` | 招待モーダル (LINE/Messenger/コピー) |
...
| `src/lib/invite.js` | copyInviteLink ユーティリティ |
...
| `api/journal-analysis.js` | 写真の AI 分析コメント生成 |
| `api/album.js` | アルバム全日写真取得 |
| `api/invite.js` | ペア発行(create-numbered/create-welcome), スラグ解決(resolve) |
| `api/streak.js` | 連続記録ストリーク GET/POST |
| `api/daily-theme.js` | AI話題生成 (⚠️ 認証チェック未実装・issue #46) |
| `api/family-insight.js` | 週次家族インサイト AI コメント |
...
| `api/lib/parseFirebaseServiceAccount.js` | Firebase Admin 認証情報パース |
...
pair_users/{pairId}/members/{uid}   → { joinedAt } ← Phase 1 pair-membership (2026-05)
...
pairs/{PAIR-XXXXXX}/meta/streak     → { count, lastDateKey, updatedAt }
```
...
- BrowserRouter形式（/pair/:slug/album 等、`#` なし）が全リンクで使われてるか確認
