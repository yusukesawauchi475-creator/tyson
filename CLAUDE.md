...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → RootRoute | ランディングページへリダイレクト |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | 初回訪問ランディング |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | 新pair発行 + 招待URL発行 |
| `/pair/:slug` | PairWorld (Outlet) | slug→pairId解決、子ルートへcontext提供 |
| `/pair/:slug/` | RootRoute (index) | 親→HomePage, 子→PairDailyPage |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 家族招待 |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter, AppRoutes, RootRoute (role振り分け) |
| `src/pages/HomePage.jsx` | 親のホーム画面 (緑/ピンク/紫の3カード) ⚠️L199,L212,L231: disabled API fetch残存 |
| `src/pages/PairDailyPage.jsx` | 子のホーム画面 ⚠️L136,L603,L635,L680: disabled API fetch残存 |
...
| `src/pages/WelcomePage.jsx` | 新pair発行 + 招待URL表示 (acquisition flow) |
| `src/pages/InvitePage.jsx` | 家族招待ページ (/pair/:slug/invite) |

### コンポーネント (src/components/)
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug のcontext provider (slug→pairId解決) |
...
| `src/lib/pairMembership.js` | ensureAuthAndMembership, pair membership管理 |
| `src/lib/pairSlug.js` | slug生成・解決ユーティリティ |
| `src/lib/invite.js` | copyInviteLink等招待ヘルパー |
| `src/lib/inviteShare.js` | LINE/SNS共有ヘルパー |
...
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history ⚠️L833,L1110: merge:true残存 |
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST ⚠️L553: merge:true残存 |
| `api/album.js` | アルバム全日写真取得 ⚠️initFirebaseAdmin順序依存 |
| `api/journal-analysis.js` | ジャーナル分析 ⚠️Firebase Auth未実装・pairId任意アクセス可 |
| `api/family-insight.js` | 家族インサイト生成 |
| `api/invite.js` | ペア発行(create-numbered/create-welcome), スラグ解決(resolve) |
| `api/streak.js` | 連続記録ストリーク ⚠️PAIR-DEMOTEST書き込みブロック欠落 |
...
| `api/lib/pair-access.js` | isPairAllowed, isTysonOnlyBlocked (DEMO_PAIR_IDS管理) |
| `api/_disabled/` | 無効化済みAPI (analyze.js, analysis-comment.js) |
