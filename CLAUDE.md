...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → RootRoute | last slug あり→/pair/:slug リダイレクト, 無→ランディング |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | 初回訪問ランディング |
| `/facilities` | FacilitiesPage | 介護施設向けランディング |
| `/welcome` | WelcomePage | 新規ユーザー向けウェルカム（lang prop 未受け取りバグあり） |
| `/pair/:slug` | PairWorld (Outlet) | slug→pairId を Firestore で解決してコンテキスト提供 |
| `/pair/:slug/` | RootRoute | 役割別ホーム (親→HomePage / 子→PairDailyPage) |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待ページ（lang 'ja' 固定バグあり） |

## ペアの仕組み
- 公開URL: `humfamily.com/pair/{8文字スラグ}` (例: /pair/abc23xyz)
- Vercel SPA rewrite: `/pair/*` → `index.html` → React BrowserRouter
- クライアントサイド解決: `PairWorld` コンポーネントが Firestore `pair_numbers/{slug}` を読み取り `pairId` を取得
- Firestore: `pair_numbers/{slug}` → `{ pairId }` (例: PAIR-H58HTP)
- 内部ID `PAIR-XXXXXX` は URL に露出しない
- slug は `pairSlug.js` の `generateSlug()` (Crockford Base32風, 8文字, l/o/0/1除外) 経由のみ生成
...
| `src/pages/WelcomePage.jsx` | 新規ユーザー向けウェルカム ⚠️ lang prop 未受け取り (日本語固定バグ) |
...
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug の Outlet コンテナ。Firestore で slug→pairId 解決しコンテキスト提供 |
| `src/components/AlbumCalendar.jsx` | アルバム内カレンダービュー |
...
| `src/components/LanguageSwitch.jsx` | 言語切り替え (ja/en) |
...
| `src/components/AdminAuth.jsx` | ⚠️ dead code — src/ 内に import なし |
...
| `src/lib/pairMembership.js` | ensureAuthAndMembership (pair membership claim) |
| `src/lib/unreadState.js` | 未読状態管理 (IndexedDB + Firestore) |
| `src/lib/unreadStateCore.js` | 未読状態コアロジック |
| `src/lib/dateFormat.js` | NY時間ベース dateKey 生成ヘルパー |
| `src/lib/demoPhotos.js` | PAIR-DEMOTEST 用デモ写真データ |
| `src/lib/fcm.js` | Firebase Cloud Messaging (FCMトークン登録) |
| `src/lib/voiceRole.js` | 音声ロール判定ヘルパー |
| `src/lib/listenedTracking.js` | 再生済み追跡 |
| `src/lib/invite.js` | 招待リンク生成 |
| `src/lib/inviteShare.js` | 招待シェア (Web Share API) |
| `src/lib/pairSlug.js` | slug生成 (generateSlug / findUniqueSlug, Crockford Base32) |
| `src/lib/uiCopy.js` | UI文字列定数 |
| `src/lib/shareTargets.js` | シェアターゲット定義 |
| `src/lib/indexedDB.js` | IndexedDB アクセスヘルパー |
| `src/lib/tysonThemes.js` | テーマカラー定義 |
| `src/lib/holidayBanner.js` | 祝日バナー |
| `src/lib/useAudioLevel.js` | 録音音量レベル hook |
...
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history + role切替 |
...
| `api/family-insight.js` | AI家族インサイト生成 ⚠️ pairId未指定時HTTP 200返却バグ |
| `api/journal-analysis.js` | 写真OCR解析 ⚠️ Firebase Auth なし・admin passwordのみ (Critical) |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | isPairAllowed / isTysonOnlyBlocked 共有ヘルパー |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase Service Account パース |
| `api/_disabled/` | 無効化済み API (upload.js / analyze.js 等) |
...
## Firestore 構造
```
pair_numbers/{slug}              → { pairId, memo, createdAt [, deactivated, migratedTo] }
...
pair_media/{pairId}/days/{dateKey}
                                 → { parent: { audioPath[], latestAudioPath, seenAt, ... }, child: {...} }
pair_users/{pairId}/parentDevices/{docId}  → { fcmToken, updatedAt }
pair_users/{pairId}/childDevices/{docId}   → { fcmToken, updatedAt } ⚠️ Firestoreルール未設定
pair_members/{pairId}/role_history/{autoId} → { uid, fromRole, toRole, reason, recordedAt }
journal/{pairId}/months/{YYYY-MM}/days/{YYYY-MM-DD}
                                 → { roleData: { parent: { generic_images: [...], journal_image: {...} }, child: {...} } }
```
...
journal/{pairId}/{YYYY-MM}/{YYYY-MM-DD}/{role}/journal_image/page-0{N}.{ext}
```
