...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → LandingPage | 最後の slug がある場合 `/pair/:slug` へリダイレクト |
| `/pair/:slug` | PairWorld (Outlet) | pair context provider (slug解決・認証・membership確認) |
| `/pair/:slug/` | RootRoute | role振り分け → HomePage / PairDailyPage / RoleSelectPage |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待ページ |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 施設向けランディングページ |
...
| `src/pages/FacilitiesPage.jsx` | 施設向けランディングページ (アニメーション付き) |
| `src/pages/WelcomePage.jsx` | オンボーディング画面 |
| `src/pages/InvitePage.jsx` | 招待リンク共有ページ |
| `src/components/PairWorld.jsx` | `/pair/:slug` の context provider (slug解決・認証・membership) |
...
| `src/components/UploadErrorModal.jsx` | アップロードエラー + retry モーダル |
...
| `src/lib/firebase.js` | Firebase初期化, getIdTokenForApi (匿名認証 + self-heal), logAuthSelfHealEvent |
| `src/lib/pairMembership.js` | ensureAuthAndMembership (pair membership claim + auth self-heal) |
| `src/lib/pairDaily.js` | getUserRole, setUserRole, uploadAudio, fetchAudio 等 |
...
| `src/lib/pairSlug.js` | slug バリデーション・フォーマット |
...
| `src/lib/inviteShare.js` | 招待シェア UI ロジック |
| `src/lib/uiCopy.js` | UI コピーテキスト (多言語) |
| `src/lib/shareTargets.js` | SNS シェアターゲット |
...
| `api/daily-theme.js` | AI話題生成 (パーソナライズ対応) |
| `api/family-insight.js` | 家族インサイトコメント生成 (AI) |
| `api/journal-analysis.js` | 写真日記分析 (AI) |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | pair アクセス制御共通ロジック (isPairAllowed 等) |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase Admin 初期化ヘルパー |
| `api/_disabled/` | 無効化済み関数群 (analyze, analysis-comment 等) |
...
pair_users/{pairId}/parentDevices/{deviceId} → { fcmToken, updatedAt }
...
- BrowserRouter形式（/path）が全リンクで使われてるか確認（`/#/` 形式は旧仕様、残存箇所は修正対象）
