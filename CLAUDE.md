...
| `/welcome` | WelcomePage | オンボーディング/ウェルカム |
| `/pair/:slug` | PairWorld (Outlet) | ペアコンテキスト (slug でペア識別) |
...
| `src/pages/WelcomePage.jsx` | オンボーディング/招待受領後のウェルカム画面 |
...
| `api/family-insight.js` | ペアのAI家族インサイト生成 |
| `api/journal-analysis.js` | 写真日記のAI分析 (管理者専用) |
...
- BrowserRouter形式（/pair/:slug/album 等）が全リンクで使われてるか確認（HashRouter /#/ 形式は廃止）
