...
| `/welcome` | WelcomePage | DEMO CTA後の新ペア発行フロー |
| `/pair/:slug` | PairWorld (Outlet) | ペア世界 (slug → pairId 解決) |
| `/pair/:slug/` | RootRoute | 親/子ホーム画面 (role振り分け) |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | ペア招待フロー |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouterルーティング, PairWorld Outlet, AppRoutes |
...
| `src/pages/WelcomePage.jsx` | DEMO CTA後の新ペア自動発行 + LINE招待フロー |
| `src/pages/InvitePage.jsx` | ペア招待フロー (招待リンク生成・シェア) |
...
| `src/lib/pairMembership.js` | ペアメンバーシップ claim・検証 (Phase 1) |
| `src/lib/pairSlug.js` | slug 生成 (Crockford Base32, generateSlug) |
| `src/lib/unreadState.js` | 未読 voice 状態取得 (Firestore + localStorage) |
| `src/lib/unreadStateCore.js` | 未読状態計算ロジック (純粋関数) |
| `src/lib/listenedTracking.js` | localStorage ベース per-item 既聴状態管理 |
| `src/lib/voiceRole.js` | voice 表示 role 決定 (immutable 訂正追記モデル) |
| `src/lib/holidayBanner.js` | lang 別祝日バナー + 14日カウントダウン |
| `src/lib/inviteShare.js` | 招待テキスト生成 (LINE/メール/コピー) |
| `src/lib/shareTargets.js` | 招待 share 宛先一覧 (LINE等) |
| `src/lib/tysonThemes.js` | AI話題 Fallback ハードコードテーマ一覧 |
| `src/lib/uiCopy.js` | UI文言共通化 (i18n経由) |
| `src/lib/deployHealthCheck.js` | デプロイ健全性チェック (BUILD_TIME/SHA確認) |
| `src/lib/indexedDB.js` | IndexedDB 音声バックアップ保存・取得 |
| `src/lib/demoPhotos.js` | デモ用写真 data (lang 別, 純粋関数) |
| `src/lib/dateFormat.js` | 日付表示フォーマット (デバイス時刻 JST 基準) |
| `src/lib/fcm.js` | FCM push通知トークン登録 (未使用: registerForPush) |
...
| `api/admin-reset.js` | 管理リセット (journal dayドキュメント削除) |
| `api/admin-restore.js` | 管理復元 (スナップショットから復元) |
| `api/admin-pairs.js` | ペアダッシュボード (30日間アクティビティ) |
| `api/family-insight.js` | 家族インサイト生成 (7日分ルックバック) |
| `api/journal-analysis.js` | 日記分析・コメント生成 (X-Admin-Password認証のみ) |
...
- BrowserRouter形式（/pair/:slug/album 等）が全リンクで使われてるか確認（HashRouter形式の /#/ は廃止）
