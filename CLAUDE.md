...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → LandingPage | hum_last_slug あり → /pair/:slug リダイレクト、なし → ランディング |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けランディング |
| `/welcome` | WelcomePage | DEMO CTA 経由新規 pair 発行 |
| `/pair/:slug` | PairWorld (Outlet) | pair world ルートネスト |
| `/pair/:slug` (index) | RootRoute → HomePage / PairDailyPage / RoleSelectPage | role 振り分け |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter + PairWorld ネスト, RootOrLanding, RootRoute (role振り分け) |
| `src/pages/HomePage.jsx` | 親のホーム画面 (声・写真・streak カード) |
...
| `src/pages/WelcomePage.jsx` | DEMO CTA 経由新規 pair 発行 + LINE シェア |
| `src/pages/InvitePage.jsx` | 招待リンク共有ページ |

### フロントエンド (Components)
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug Outlet — Firestore slug 解決 + context 供給 |
| `src/components/DailyPromptCard.jsx` | 今日の話題pill (AI話題 + 別の話題ボタン) |
| `src/components/VoiceLibrary.jsx` | 声の履歴一覧 (アルバム声タブ用) |
| `src/components/AlbumCalendar.jsx` | アルバムカレンダービュー (月別日付選択) |
...
| `src/components/Visualizer.jsx` | 録音/再生波形 visualizer (button overlay) |
...
| ファイル | 役割 |
|---------|------|
| `src/lib/pairDaily.js` | getPairId, getUserRole, markSeen, uploadAudio, fetchAudio, getStreak 等 |
...
| `src/lib/i18n.js` | 日英翻訳 (ja/en/es) |
| `src/lib/pairMembership.js` | ensureAuthAndMembership — auth + membership self-heal |
| `src/lib/pairSlug.js` | slug ↔ pairId 変換ユーティリティ |
| `src/lib/indexedDB.js` | IndexedDB キャッシュ (音声/写真) |
| `src/lib/fcm.js` | FCM プッシュ通知登録 (registerForPush — 未使用) |
| `src/lib/listenedTracking.js` | markVoiceListened — 既聴フラグ管理 |
| `src/lib/unreadState.js` | getPartnerUnreadState — 相手未読状態取得 |
| `src/lib/unreadStateCore.js` | 未読状態コアロジック |
| `src/lib/voiceRole.js` | getEffectiveRole — 音声ロール判定 |
| `src/lib/holidayBanner.js` | getUpcomingHoliday — 祝日バナー |
| `src/lib/inviteShare.js` | buildInviteMessage — 招待文章生成 |
| `src/lib/shareTargets.js` | LINE/SMS/コピー共有先定義 |
| `src/lib/tysonThemes.js` | ペアテーマカラー定義 |
| `src/lib/uiCopy.js` | getFinalOneLiner, getAnalysisPlaceholder — UI コピー |
| `src/lib/deployHealthCheck.js` | フロントエンドビルドハッシュ確認 |
| `src/lib/demoPhotos.js` | デモ用静的写真データ |
| `src/lib/invite.js` | buildInviteUrl — 招待URL生成 |
| `src/lib/useAudioLevel.js` | useAudioLevel フック (マイク音量取得) |
...
| `api/daily-theme.js` | AI話題生成 (pairId 任意—後述 Low バグ) |
| `api/family-insight.js` | AI family insight 生成 (7日分ルックバック) |
| `api/journal-analysis.js` | 写真AI分析 (⚠️ Firebase Auth なし—Critical バグ) |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | isTysonOnlyBlocked, isPairAllowed 共通ライブラリ |
| `api/_disabled/` | 廃止済みエンドポイント (analyze.js, analysis-comment.js) — 404 |
...
- BrowserRouter形式（/pair/:slug/...）が全リンクで使われてるか確認（旧 /#/path 形式は廃止）
