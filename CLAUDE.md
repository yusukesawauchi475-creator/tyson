...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding | 最後のslugあり→`/pair/:slug`リダイレクト, なし→LandingPage |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | 新pair発行ページ (DEMO CTA経由) |
| `/pair/:slug` | PairWorld (Outlet) | pair世界のルート (role振り分け → HomePage / PairDailyPage) |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けLP (静的テキスト日付あり ⚠️ 要更新) |
| `src/pages/WelcomePage.jsx` | 新pair発行 (DEMO CTA導線) ⚠️ lang prop未受け取り・全テキスト日本語固定 |
| `src/pages/InvitePage.jsx` | 招待ページ ⚠️ `const lang = 'ja'` 固定でprops無視 |
...
| `src/components/FamilyInsightCard.jsx` | 週次家族インサイト ⚠️ lang/pairIdを含まないキャッシュキー |
| `src/components/InviteModal.jsx` | 招待モーダル |
| `src/components/LanguageSwitch.jsx` | 言語切替UI |
| `src/components/OneYearAgoBanner.jsx` | 1年前思い出バナー (⚠️ どこからもimportされていない dead code) |
| `src/components/PairWorld.jsx` | pair Outlet wrapper (pairId解決, lang/slug context提供) |
...
| `src/lib/dateFormat.js` | NY時間 dateKey ヘルパー |
| `src/lib/demoPhotos.js` | デモ写真定義 ⚠️ date labelsが日本語固定 |
| `src/lib/deployHealthCheck.js` | デプロイ健全性チェック |
| `src/lib/fcm.js` | Firebase Cloud Messaging (プッシュ通知) |
| `src/lib/holidayBanner.js` | 祝日バナーロジック |
| `src/lib/indexedDB.js` | IndexedDB音声キャッシュ |
| `src/lib/invite.js` | 招待リンク生成・コピー |
| `src/lib/inviteShare.js` | 招待シェア (LINE等) |
| `src/lib/listenedTracking.js` | 既読追跡 |
| `src/lib/pairSlug.js` | slugバリデーション |
| `src/lib/shareTargets.js` | シェア先定義 |
| `src/lib/tysonThemes.js` | テーマ定義 |
| `src/lib/uiCopy.js` | UIコピーテキスト |
| `src/lib/unreadState.js` | 未読状態管理 |
| `src/lib/unreadStateCore.js` | 未読状態コア |
| `src/lib/useAudioLevel.js` | 音量レベル検出Hook |
...
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history ⚠️ L816/L1076 merge:true残存, L1031-1037 pairId検証順序バグ |
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST ⚠️ L536 merge:true残存, L405 PAIR-DEMOTEST文字列直比較 |
| `api/album.js` | アルバム全日写真取得 |
| `api/invite.js` | ペア発行(create-numbered/create-welcome), スラグ解決(resolve) |
| `api/streak.js` | 連続記録ストリーク ⚠️ POST で PAIR-DEMOTEST への書き込みブロックなし |
| `api/daily-theme.js` | AI話題生成 ⚠️ verifyIdToken未実装 — 認証なしで任意pairIdのデータ読み取り可能 |
| `api/family-insight.js` | 家族インサイト生成 ⚠️ !pairId時にHTTP 200返却, 日付計算がUTC基準 |
| `api/journal-analysis.js` | 日記分析 ⚠️ merge:true残存 (L120, L176) |
| `api/admin-reset.js` | 管理リセット ⚠️ merge:true残存 (L151) |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | isPairAllowed(), verifyIdToken(), READ_ONLY/DEMO pair定数 |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase service account JSON パース |
| `api/_disabled/` | 無効化済みAPI群 (analyze, analysis-comment, notify等) ⚠️ フロントに fetch 参照残存 |
...
pair_users/{pairId}/members/{uid}  → { uid, role, joinedAt } (pair membership)
pair_media/{pairId}/days/{dateKey}  → { parent: { audioPath[], latestAudioPath, ... }, child: {...} }
pairs/{pairId}/meta/streak         → { count, lastDateKey, firstDateKey, updatedAt }
...
- BrowserRouter形式（/pair/:slug 等）が全リンクで使われてるか確認
