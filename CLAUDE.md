...
| `/welcome` | WelcomePage | 新pair発行・LINE共有オンボーディング |
| `/pair/:slug` | PairWorld | pair コンテキスト (outlet) |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | ルーティング, RootRoute (role振り分け), AppRoutes |
...
| `src/pages/WelcomePage.jsx` | 新pair発行・LINE共有オンボーディング (i18n未対応) |
| `src/pages/InvitePage.jsx` | 招待フロー (lang固定'ja') |
...
| `src/components/PairWorld.jsx` | pair コンテキストProvider (outlet) |
| `src/components/PwaInstallBanner.jsx` | Android PWAインストールバナー |
| `src/components/RoleBadge.jsx` | role表示バッジ |
...
| `src/lib/inviteShare.js` | LINE/SNS 共有ヘルパー |
...
| `src/lib/pairMembership.js` | pair メンバーシップ管理 |
...
| `src/lib/useAudioLevel.js` | 録音音声レベルhook |
...
| `api/family-insight.js` | 家族インサイト生成 (UTC日付バグあり) |
| `api/journal-analysis.js` | ジャーナル分析 (Firebase Auth未実装) |
