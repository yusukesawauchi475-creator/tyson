...
| `/pair/:slug` | PairWorld | ペアコンテキストの親ルート |
...
| `src/pages/WelcomePage.jsx` | ウェルカム画面 (新規ユーザー向け) |
...
| `api/family-insight.js` | ペアの週次/月次インサイト生成 (AI) |
| `api/journal-analysis.js` | 日記画像の OCR + AI 解析 (admin専用) |
...
- BrowserRouter形式（/path）が全リンクで使われてるか確認（HashRouter形式 /#/ は廃止）
