...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → RootRoute | ランディング or ホーム（role判定） |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けLP |
| `/welcome` | WelcomePage | 招待後ウェルカム画面 |
| `/pair/:slug` | PairWorld (layout) | ペア空間レイアウト (slug解決) |
| `/pair/:slug` (index) | RootRoute | ペアホーム (role振り分け) |
...
| `api/daily-theme.js` | AI話題生成 (⚠️ 認証なし — issue #47) |
| `api/family-insight.js` | AI生成ファミリーインサイト (毎日1回) |
| `api/journal-analysis.js` | 写真分析 (AI) |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | pairId アクセス権判定 (isPairAllowed, DEMO_PAIR_IDS) |
| `api/_disabled/` | 無効化済み API 群 (analyze.js, analysis-comment.js 等) — フロント参照が残存 |
...
pairs/{PAIR-XXXXXX}/meta/streak → { currentStreak, lastActiveDate, ... }
...
- BrowserRouter形式（/pair/:slug/album 等）が全リンクで使われてるか確認（`/#/` 旧形式が混入していないか）
