...
| `/pair/:slug` | PairWorld | pair レイアウト (slug → pairId 解決・メンバー検証) |
| `/pair/:slug` (index) | RootRoute | 親/子ホーム画面 (role 振り分け) |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | ルーティング, AppRoutes, RootRoute (role振り分け), lang検出 |
...
| `src/pages/WelcomePage.jsx` | ウェルカム画面 (オンボーディング) |
| `src/pages/InvitePage.jsx` | 招待ページ (SNS/Messenger シェア) |
| `src/components/PairWorld.jsx` | `/pair/:slug` レイアウト — slug解決・membership検証・OutletContext提供 |
...
| `src/components/DemoModal.jsx` | デモ時の書き込みブロックモーダル |
...
| `src/lib/pairMembership.js` | pair membership claim/verify (Phase 1) |
| `src/lib/firebase.js` | Firebase初期化, getIdTokenForApi (匿名認証) |
| `src/lib/i18n.js` | 日英西翻訳 |
| `src/lib/invite.js` | 招待リンク生成 |
| `src/lib/inviteShare.js` | SNS/Messenger シェアヘルパー |
| `src/lib/voiceRole.js` | 再生対象ロール判定 |
| `src/lib/unreadState.js` | 未読状態管理 (public API) |
| `src/lib/unreadStateCore.js` | 未読状態計算コア |
| `src/lib/listenedTracking.js` | 再生済みトラッキング |
| `src/lib/pairSlug.js` | slug ↔ pairId 変換ヘルパー |
| `src/lib/fcm.js` | Firebase Cloud Messaging (プッシュ通知) |
| `src/lib/indexedDB.js` | IndexedDB キャッシュ (音声バッファ) |
| `src/lib/uiCopy.js` | UIコピーテキスト定数 |
| `src/lib/shareTargets.js` | シェアターゲット定義 |
| `src/lib/tysonThemes.js` | pair別カラーテーマ |
| `src/lib/useAudioLevel.js` | マイクレベル計測 React hook |
...
| `api/daily-theme.js` | AI話題生成 (⚠️ 認証なし — 要修正) |
| `api/family-insight.js` | AI生成ファミリーインサイト |
| `api/journal-analysis.js` | 日記AI分析 |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | isPairAllowed() — pair membership検証共通ロジック |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase Admin 初期化ヘルパー |
| `api/_disabled/` | 無効化済み関数 (analyze, analysis-comment, convert-audio 等 13件) |
...
## Firestore 構造
```
pair_numbers/{slug}              → { pairId, deactivated, memo, createdAt }
pairs/{PAIR-XXXXXX}              → { pairId, number(slug), memo, createdAt }
pair_users/{pairId}/members/{uid}        → { role, claimedAt }  ← Phase 1 membership
pair_users/{pairId}/parentDevices/{deviceId} → { token, updatedAt }  ← FCM tokens
