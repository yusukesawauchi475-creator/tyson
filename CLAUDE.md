...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding | pairId無→ランディング |
| `/landing` | LandingPage | 初回訪問ランディング |
| `/demo` | DemoPage | デモ体験 |
| `/admin` | AdminPage | 管理画面 |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | ウェルカム/オンボーディング |
| `/pair/:slug` | PairWorld (Outlet) | pair文脈プロバイダ (slug→pairId解決) |
| `/pair/:slug/` | RootRoute | 親→HomePage, 子→PairDailyPage |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待ページ |

## ペアの仕組み
- 公開URL: `humfamily.com/pair/{8文字スラグ}` (例: /pair/ulf1q6ab)
- PairWorld コンポーネントが client-side で Firestore `pair_numbers/{slug}` → `pairId` 解決
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | ルーティング定義, AppRoutes, RootRoute (role振り分け) |
...
| `src/pages/WelcomePage.jsx` | ウェルカム/オンボーディング |
| `src/pages/InvitePage.jsx` | 招待リンク共有 |

### コンポーネント (src/components/)
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug Outlet コンテキストプロバイダ (slug→pairId解決) |
...
| `src/components/FamilyInsightCard.jsx` | 家族インサイトカード (AI生成) |
...
| `src/components/AdminAuth.jsx` | ⚠️ dead code (import なし) |

### lib (src/lib/)
| ファイル | 役割 |
|---------|------|
| `src/lib/firebase.js` | Firebase初期化, getIdTokenForApi (匿名認証) |
| `src/lib/pairDaily.js` | markSeen, uploadAudio, fetchAudio 等 |
...
| `src/lib/unreadState.js` | 未読状態管理 (Firestore購読) |
| `src/lib/unreadStateCore.js` | 未読状態コアロジック |
| `src/lib/listenedTracking.js` | 音声再生済み追跡 |
| `src/lib/indexedDB.js` | オフラインキャッシュ (IndexedDB) |
| `src/lib/fcm.js` | FCMプッシュ通知 |
| `src/lib/inviteShare.js` | 招待リンク共有ロジック |
| `src/lib/invite.js` | 招待関連ユーティリティ |
| `src/lib/shareTargets.js` | Web Share Target |
...
| `src/lib/useAudioLevel.js` | 音量レベル検出フック |
...
| `api/family-insight.js` | AI家族インサイト生成 (OpenAI) |
| `api/journal-analysis.js` | ⚠️ 写真OCR+AI分析 (要認証強化・既知バグあり) |
...
| `api/lib/parseFirebaseServiceAccount.js` | Firebase SA パース共通ロジック |
| `api/_disabled/` | 無効化済み関数置き場 (analyze.js, analysis-comment.js 等) |
