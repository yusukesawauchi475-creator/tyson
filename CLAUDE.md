...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding | slug保存済→/pair/:slug にリダイレクト, 無→LandingPage |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | ウェルカム画面 |
| `/pair/:slug` | PairWorld (outlet) | slug解決・pairId提供 |
| `/pair/:slug/` (index) | RootRoute | role → RoleSelectPage / HomePage / PairDailyPage |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter ルーティング定義, RootOrLanding, RootRoute (role振り分け) |
| `src/pages/HomePage.jsx` | 親のホーム画面 (録音・写真・アルバム) |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けページ |
| `src/pages/WelcomePage.jsx` | ウェルカム画面 (⚠️i18n未実装) |
| `src/pages/InvitePage.jsx` | 招待ページ |

### フロントエンド コンポーネント
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug 配下のレイアウト (slug解決・pairId・lang を outlet context で提供) |
...
| `src/components/InviteModal.jsx` | 招待モーダル (LINE + clipboard) |
| `src/components/DemoModal.jsx` | デモCTAモーダル (write系操作タップ時) |
| `src/components/RoleBadge.jsx` | 役割バッジ表示 |
| `src/components/Visualizer.jsx` | 録音中波形ビジュアライザー |
| `src/components/UploadErrorModal.jsx` | アップロードエラーモーダル (リトライ付き) |
| `src/components/LanguageSwitch.jsx` | 言語切替コンポーネント |
| `src/components/FamilyInsightCard.jsx` | ファミリーインサイトカード |
...
| `src/components/AdminAuth.jsx` | ⚠️ dead code (import箇所なし) |

### フロントエンド lib
| ファイル | 役割 |
|---------|------|
| `src/lib/pairDaily.js` | getUserRole, setUserRole, uploadAudio, fetchAudioForPlayback, markSeen, getDateKey 等 |
...
| `src/lib/i18n.js` | 日英西翻訳 (t(), getMonthName()) |
| `src/lib/pairMembership.js` | ensureAuthAndMembership (自己ヒール付きメンバーシップ確認) |
| `src/lib/pairSlug.js` | slug管理 |
| `src/lib/invite.js` | 招待URL生成 (buildInviteUrl) |
| `src/lib/inviteShare.js` | 招待シェアテキスト生成 (buildInviteMessage) |
| `src/lib/dateFormat.js` | デプロイ日時フォーマット, getBuildHash |
| `src/lib/listenedTracking.js` | 音声聴取済みトラッキング (markVoiceListened) |
| `src/lib/unreadState.js` | 未読状態管理 (getPartnerUnreadState) |
| `src/lib/unreadStateCore.js` | 未読状態コアロジック |
| `src/lib/voiceRole.js` | 音声ロール判定 (getEffectiveRole) |
| `src/lib/useAudioLevel.js` | 録音音声レベル取得 hook |
| `src/lib/fcm.js` | FCMプッシュ通知登録 |
| `src/lib/indexedDB.js` | IndexedDB キャッシュ |
| `src/lib/demoPhotos.js` | デモ写真データ定義 |
| `src/lib/holidayBanner.js` | 直近祝日バナー (getUpcomingHoliday) |
| `src/lib/deployHealthCheck.js` | デプロイヘルスチェック |
| `src/lib/uiCopy.js` | UI文言 (getFinalOneLiner, getAnalysisPlaceholder) |
...
| `api/family-insight.js` | ファミリーインサイトコメント生成 (⚠️pairId欠落時200返却) |
| `api/journal-analysis.js` | ジャーナル分析・OCR (⚠️Firebase Auth未実装・CRITICAL) |
| `api/admin-reset.js` | 管理リセット (⚠️PAIR-DEMOTEST書き込みブロック欠落) |
| `api/admin-restore.js` | 管理復元 (⚠️PAIR-DEMOTEST書き込みブロック欠落) |
...
| `api/_disabled/` | 無効化済みAPI (analyze.js / analysis-comment.js) — フロントからの参照残存 |
...
- BrowserRouter形式（/pair/:slug/...）が全リンクで使われてるか確認（/#/ 形式は廃止済み）
