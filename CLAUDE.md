...
- **i18n**: ja / en / es 3言語（`src/lib/i18n.js` の `t(lang, key)` + `getMonthName(lang, month)` 経由）
...
| `/welcome` | WelcomePage | 招待受け取り側の初回ウェルカム画面 |
...
- Vercel rewrite → SPA エントリポイント（`/pair/:slug` はクライアント側解決）
- PairWorld コンポーネントが Firestore `pair_numbers/{slug}` を直接読み取り → `pairId` 解決 (例: PAIR-H58HTP)
- 解決した `{ pairId, slug, lang }` を Outlet context として子ルートに伝播
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter ルーティング, AppRoutes, RootOrLanding, RootRoute (role振り分け) |
...
| `src/pages/WelcomePage.jsx` | 招待受け取り側の初回ウェルカム画面 (⚠️ i18n未対応: lang prop受け取らず日本語固定) |
| `src/pages/InvitePage.jsx` | 招待リンク共有 (/pair/:slug/invite) (⚠️ lang='ja'ハードコード: issue #21参照) |
| `src/components/PairWorld.jsx` | /pair/:slug の slug→pairId 解決コンテキスト (Outlet context: { pairId, slug, lang }) |
| `src/components/DailyPromptCard.jsx` | 今日の話題pill (AI話題 + 別の話題ボタン) |
| `src/components/LanguageSwitch.jsx` | 🇯🇵/🇺🇸/🇪🇸 言語切替 (URLクエリ ?lang + localStorage hum_lang/hum_country 永続化) |
| `src/components/VoiceLibrary.jsx` | 声の履歴一覧 (アルバム声タブ用) |
| `src/components/AlbumCalendar.jsx` | アルバムカレンダービュー |
| `src/components/DemoModal.jsx` | デモ用モーダル (PAIR-DEMOTEST write block時CTA) |
| `src/components/FamilyInsightCard.jsx` | 家族インサイトカード |
| `src/components/InviteModal.jsx` | 招待モーダル |
| `src/components/OneYearAgoBanner.jsx` | 1年前の記憶バナー |
| `src/components/RoleBadge.jsx` | role表示バッジ (tap でrole変更) |
| `src/components/UploadErrorModal.jsx` | アップロードエラーモーダル |
| `src/components/Visualizer.jsx` | 録音/再生中 button overlay 半透明波形 |
| `src/components/PwaInstallBanner.jsx` | Android PWAインストールバナー |
| `src/components/WeeklySummary.jsx` | 週次サマリー (日曜のみ) |
| `src/lib/pairDaily.js` | uploadAudio, fetchAudio, markSeen, getDateKey, updateStreak 等 |
| `src/lib/journal.js` | 写真アップロード, fetchTodayJournalMeta, fetchAlbum |
| `src/lib/firebase.js` | Firebase初期化, getIdTokenForApi (匿名認証) |
| `src/lib/i18n.js` | ja/en/es 3言語翻訳: t(lang,key), getMonthName(lang,month) |
| `src/lib/dateFormat.js` | 日付フォーマットユーティリティ, formatDeployedAtLocal, getBuildHash |
...
| `src/lib/pairSlug.js` | スラグ生成ユーティリティ |
| `src/lib/shareTargets.js` | SNSシェアターゲット定義 |
| `src/lib/tysonThemes.js` | テーマ定義 |
| `src/lib/uiCopy.js` | getFinalOneLiner, getAnalysisPlaceholder 等UIコピー |
| `src/lib/useAudioLevel.js` | 録音レベル取得カスタムフック |
| `src/lib/voiceRole.js` | voice role判定ユーティリティ |
...
| `api/daily-theme.js` | AI話題生成 (⚠️ verifyIdToken未実装: issue #21参照) |
| `api/family-insight.js` | 家族インサイト生成 |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/_disabled/journal-analysis.js` | 音声解析 (無効化済み、参照コードが残存: issue #21参照) |
...
### 9. JP/EN/ES文字列チェック
- esモード時に日本語ハードコード文字列がないか確認：
  grep -rn "lang === 'en' ?" src/pages/ | head -30  # 二項 en/ja チェックは es で日本語fallback
...
- LanguageSwitchが全ページ(HomePage/PairDailyPage)に表示されてるか確認
