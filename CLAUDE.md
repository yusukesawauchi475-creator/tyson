...
| `/pair/:slug` | PairWorld → RootRoute | pair コンテキスト (role振り分け) |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | ルーティング (BrowserRouter), PairWorld context, RootRoute (role振り分け) |
...
| `api/family-insight.js` | ファミリーインサイト (AI解析コメント) |
| `api/journal-analysis.js` | 日記エントリ分析 |
...
- BrowserRouter形式（/pair/:slug 等）が全リンクで使われてるか確認（HashRouter形式 /#/ は使用禁止）
