...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → LandingPage or /pair/:slug | pair未確定→ランディング |
| `/admin` | AdminPage | 管理画面 (ペア発行, ダッシュボード) |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | 初回訪問ランディング |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | ウェルカム / ペア作成 |
| `/pair/:slug` | PairWorld (Outlet) | ペア世界のルートレイアウト |
| `/pair/:slug/` | RootRoute → HomePage or PairDailyPage | role振り分けホーム |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待リンク生成 |
...
| `src/pages/HomePage.jsx` | 親のホーム画面 (緑/ピンク/紫の3カード) ⚠️ 無効API fetch残存 L199,L212,L231 |
| `src/pages/PairDailyPage.jsx` | 子のホーム画面 ⚠️ 無効API fetch残存 L136,L603,L635,L680 |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けページ (Formspree フォーム) |
| `src/pages/WelcomePage.jsx` | ウェルカム / ペア作成 (/api/invite?action=create-welcome) |
| `src/pages/InvitePage.jsx` | 招待リンク生成 (slug → invite URL) |

### コンポーネント (src/components/)
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug のレイアウトラッパー (slug→pairId解決, Outlet) |
...
| `src/components/AdminAuth.jsx` | 管理画面認証 |
...
| `api/album.js` | アルバム全日写真取得 ⚠️ isPairAllowed がinitFirebaseAdmin前 L121 |
...
| `api/family-insight.js` | AIファミリーインサイト生成 (Bearer認証+isPairAllowed) |
| `api/journal-analysis.js` | 写真OCR+AI解析 ⚠️ **Critical: Firebase Auth なし、X-Admin-Passwordのみ** |
...
| `api/lib/pair-access.js` | isPairAllowed, isTysonOnlyBlocked 共通ライブラリ |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase SA JSON パーサー |
| `api/_disabled/` | 無効化済みAPI群 (analyze, analysis-comment, notify 等) |
