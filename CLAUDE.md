...
| `/pair/:slug` | PairWorld → RootRoute | スラグ経由のペアコンテキスト |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待ページ |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ |
| `/landing` | LandingPage | ランディングページ |
| `/welcome` | WelcomePage | DEMO CTA 経由の新規ペア発行 |
| `/facilities` | FacilitiesPage | 介護施設向け導線 |
...
| `src/pages/WelcomePage.jsx` | DEMO CTA 経由の新規ペア発行 (⚠️ lang prop 未実装・日本語固定) |
| `src/pages/FacilitiesPage.jsx` | 介護施設向け導線 |
...
| `api/family-insight.js` | 家族インサイト (⚠️ UTC日付バグあり) |
| `api/journal-analysis.js` | ジャーナル分析 (⚠️ Firebase Auth なし・要確認) |
...
- BrowserRouter形式（/pair/:slug/...）が全リンクで使われてるか確認（`/#/` ハッシュURLは廃止）
