...
| `/welcome` | WelcomePage | ウェルカムページ（i18n未実装・JP固定） |
| `/pair/:slug` | PairWorld (Outlet) | pair世界のルート |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter ルーティング, PairWorld Outlet, RootRoute (role振り分け) |
...
| `src/pages/WelcomePage.jsx` | ウェルカム画面 ⚠️ i18n未実装・全テキストJP固定 |
| `src/pages/InvitePage.jsx` | 招待リンク共有ページ |

### components
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug Outlet + slug→pairId解決 |
...
| `api/album.js` | アルバム全日写真取得 ⚠️ isPairAllowed がinitFirebaseAdmin()より前 |
| `api/invite.js` | ペア発行(create-numbered), スラグ解決(resolve) |
| `api/streak.js` | 連続記録ストリーク ⚠️ PAIR-DEMOTEST書き込みブロック欠落 |
| `api/daily-theme.js` | AI話題生成 ⚠️ pairId欠落時200返却・monthKey UTC使用 |
| `api/admin-reset.js` | 管理リセット ⚠️ PAIR-DEMOTEST書き込みブロック欠落 |
| `api/admin-restore.js` | 管理復元 ⚠️ PAIR-DEMOTEST書き込みブロック欠落 |
| `api/admin-pairs.js` | ペアダッシュボード ⚠️ 30日カレンダー日付 UTC使用 |
| `api/family-insight.js` | 家族インサイトコメント生成 ⚠️ pairId欠落時200返却・日付 UTC使用 |
| `api/journal-analysis.js` | 写真OCR/分析 ⚠️ Firebase Auth なし・pairId任意でFirestore読み書き可能 |
| `api/lib/pair-access.js` | isPairAllowed, isTysonOnlyBlocked 等 共通認証ロジック |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase SA パース |
| `api/_disabled/` | 無効化済み関数群（analysis-comment.js / analyze.js 等）⚠️ フロント側fetch呼び出しが残存 |
