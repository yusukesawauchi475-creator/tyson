...
| `/pair/:slug` | PairWorld (Outlet) | スラグ解決・pair context 提供 |
| `/pair/:slug/` | RootRoute | role→HomePage(親) / PairDailyPage(子) / RoleSelectPage |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter ルーティング, RootOrLanding, AppRoutes, PairWorld Outlet |
...
| `src/pages/FacilitiesPage.jsx` | 施設向けランディング (デモアニメーション付き) |
| `src/pages/WelcomePage.jsx` | 新規ユーザーオンボーディング (招待リンク表示) |
...
| `src/components/PairWorld.jsx` | pair slug 解決・pairId/slug を Outlet context で子に渡す |
...
| `src/lib/shareTargets.js` | 共有先ターゲット定義 |
...
| `src/lib/useAudioLevel.js` | 録音音声レベル hook |
...
| `api/invite.js` | ペア発行(create-numbered/create-welcome), スラグ解決(resolve), slug maskあり |
...
| `api/journal-analysis.js` | 日記分析 (X-Admin-Password認証のみ・注意) |
| `api/lib/pair-access.js` | isPairAllowed / isTysonOnlyBlocked 共通関数 |
