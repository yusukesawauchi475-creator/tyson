...
| `/pair/:slug` | PairWorld (Outlet) | ペアワールド (slug で pair 識別) |
| `/pair/:slug/` | RootRoute | 親/子ホーム画面 (role で分岐) |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待シェア画面 |
...
| `src/pages/HomePage.jsx` | 親のホーム画面 ⚠️ _disabled API呼び出し残存 (L199,L212,L231)_ |
| `src/pages/PairDailyPage.jsx` | 子のホーム画面 ⚠️ _disabled API呼び出し残存 (L136,L603,L635,L680)_ |
...
| `src/pages/WelcomePage.jsx` | ペア参加ウェルカム画面 |
| `src/pages/InvitePage.jsx` | 招待シェア画面 |

### コンポーネント
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | slug→pairId解決, pair membership gate, Outlet |
...
| `src/components/AlbumCalendar.jsx` | アルバム日付カレンダービュー |
...
| `src/components/AdminAuth.jsx` | 管理者認証UI |
...
| `src/lib/listenedTracking.js` | 音声視聴済みトラッキング |
...
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history ⚠️ _merge:true残存 L833,L1110_ |
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST ⚠️ _merge:true残存 L553_ |
| `api/album.js` | アルバム全日写真取得 ⚠️ _isPairAllowed/initFirebaseAdmin順序不安定_ |
| `api/invite.js` | ペア発行(create-numbered), スラグ解決(resolve) |
| `api/streak.js` | 連続記録ストリーク ⚠️ _PAIR-DEMOTEST書き込みブロック欠落_ |
| `api/daily-theme.js` | AI話題生成 |
| `api/family-insight.js` | AI家族インサイト生成 |
| `api/journal-analysis.js` | 写真分析 ⚠️ **[Critical] Firebase Auth なし・任意pairId書き込み可** |
| `api/admin-reset.js` | 管理リセット ⚠️ _PAIR-DEMOTEST書き込みブロック欠落_ |
| `api/admin-restore.js` | 管理復元 ⚠️ _PAIR-DEMOTEST書き込みブロック欠落_ |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/` | 共通ライブラリ (pair-access.js等) |
| `api/_disabled/` | 無効化済み関数 (analyze.js, analysis-comment.js等) |
