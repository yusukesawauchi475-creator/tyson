...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding | localStorage slug有→/pair/:slug リダイレクト, 無→LandingPage |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 施設向けページ |
| `/welcome` | WelcomePage | 招待受諾オンボーディング（既知バグ: lang prop 無視）|
| `/pair/:slug` | PairWorld (Outlet) | ペア世界のルート。Firestore でpairId解決 |
| `/pair/:slug/` | RootRoute | role振り分け → HomePage / PairDailyPage / RoleSelectPage |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待（既知バグ: lang='ja' ハードコード）|

## ペアの仕組み
- 公開URL: `humfamily.com/pair/{6文字スラグ}` (例: /pair/ulf1q6)
- Vercel SPA rewrite → index.html → BrowserRouter → PairWorld
- PairWorld がクライアント側で Firestore `pair_numbers/{slug}` → `pairId` を解決
...
| `src/pages/InvitePage.jsx` | 招待受諾 (既知バグ: lang='ja' ハードコード, Phase 3 pending) |
| `src/pages/WelcomePage.jsx` | 招待受諾オンボーディング (既知バグ: lang prop 無視, 全文字列日本語固定) |

### フロントエンド (コンポーネント)
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug のルートコンポーネント。Firestore でpairId解決してOutlet提供 |
...
| `src/lib/pairMembership.js` | ペアメンバーシップ確認 (isPairMember, claimMembership) |
| `src/lib/unreadState.js` / `unreadStateCore.js` | 未読状態管理 |
| `src/lib/dateFormat.js` | 日付フォーマット (NY/JST timezone) |
| `src/lib/demoPhotos.js` | デモ用静的写真データ |
| `src/lib/invite.js` / `inviteShare.js` | 招待リンク生成・シェア |
| `src/lib/voiceRole.js` | 音声の role 判定 |
| `src/lib/pairSlug.js` | slug 操作ユーティリティ |
| `src/lib/indexedDB.js` | オフラインキャッシュ (IndexedDB) |
...
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history + role_history記録 |
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST |
| `api/album.js` | アルバム全日写真取得 |
| `api/invite.js` | ペア発行(create-numbered), スラグ解決(resolve), slug log mask実装済み |
| `api/streak.js` | 連続記録ストリーク |
| `api/daily-theme.js` | AI話題生成 (verifyIdToken + isPairAllowed 実装済み, 2026-07-02) |
| `api/family-insight.js` | 家族インサイトAI生成 (⚠️ 認証なし・要確認) |
| `api/journal-analysis.js` | ジャーナル画像OCR/AI分析 (⚠️ 認証なし — Critical未対応) |
...
| `api/_disabled/` | 無効化済みAPI (analyze, analysis-comment) — フロントのfetch呼び出しが残存 |
