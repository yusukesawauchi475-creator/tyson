...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → WelcomePage or LandingPage | 未訪問→ランディング, 訪問済み→ウェルカム |
| `/pair/:slug` | PairWorld → RoleSelect / HomePage / PairDailyPage | slug解決→role振り分け |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待ページ |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ |
| `/landing` | LandingPage | ランディングページ |
| `/welcome` | WelcomePage | ウェルカム（既訪問者向け） |
| `/facilities` | FacilitiesPage | 介護施設向け申し込みページ |
...
| `src/pages/WelcomePage.jsx` | ウェルカム画面（既訪問者・ペアURL共有用） |
| `src/pages/FacilitiesPage.jsx` | 介護施設向け申し込みフォーム |
| `src/pages/InvitePage.jsx` | 招待リンク生成・共有 |
...
| `api/family-insight.js` | ペアの週次インサイト生成 (AI) |
| `api/journal-analysis.js` | 日記・写真の分析 (Admin専用, X-Admin-Password認証) |
