...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → RootRoute | pair未選択→ランディング, pair選択済み→role振り分け |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | 初回訪問ランディング |
| `/facilities` | FacilitiesPage | 介護施設向けランディング |
| `/welcome` | WelcomePage | 新規pair発行後オンボーディング（LINE送信） |

### PairWorldネスト（`/pair/:slug/*`）
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/pair/:slug/` | PairWorld > RootRoute | slug→pairId解決, role振り分け→HomePage or PairDailyPage |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouterルーティング, PairWorld, RootRoute (role振り分け) |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けランディング (アニメーションデモ付き) |
| `src/pages/WelcomePage.jsx` | 新規pair発行後オンボーディング (LINE送信フロー) |
| `src/pages/InvitePage.jsx` | 招待リンク発行・共有 |

### Components
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug のレイアウトOutlet, slug→pairId解決 |
...
| `src/components/DemoModal.jsx` | PAIR-DEMOTEST操作時のデモ案内モーダル |
| `src/components/FamilyInsightCard.jsx` | ファミリーインサイト表示カード |
...
| ファイル | 役割 |
|---------|------|
| `src/lib/pairDaily.js` | getUserRole, setUserRole, uploadAudio, fetchAudio, getStreak 等 |
...
| `src/lib/pairMembership.js` | ensureAuthAndMembership (Auth + pair membership確認) |
| `src/lib/pairSlug.js` | slug解決ユーティリティ |
| `src/lib/indexedDB.js` | 音声オフラインキャッシュ (IndexedDB) |
| `src/lib/fcm.js` | FCMプッシュ通知登録 (registerForPush, 現状未呼び出し) |
...
| `src/lib/uiCopy.js` | UIコピー文言 |
| `src/lib/deployHealthCheck.js` | デプロイヘルスチェック |
| `src/lib/demoPhotos.js` | デモ用写真データ |
| `src/lib/invite.js` | 招待フロントエンドロジック |
| `src/lib/useAudioLevel.js` | 録音音量レベルhook |
...
| `api/daily-theme.js` | AI話題生成 (pair membership確認付き) |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/family-insight.js` | ファミリーインサイト生成 (AI) |
| `api/journal-analysis.js` | 日記分析 (AI, X-Admin-Password認証のみ) |
| `api/lib/pair-access.js` | isPairAllowed, isTysonOnlyBlocked (pair membership共通ロジック) |
...
- BrowserRouter形式（/pair/:slug/path）が全リンクで使われてるか確認（旧HashRouter /#/ 形式は禁止）
