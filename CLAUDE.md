...
| `/pair/:slug` | PairWorld (Outlet) | ペアワールド (Outlet ネスト) |
| `/pair/:slug/` | RootRoute | pairId有→ホーム (index route) |
...
| `src/pages/AdminPage.jsx` | 管理画面 (ペア発行, ダッシュボード) — 全テキスト日本語ハードコード (i18n未対応) |
| `src/pages/RoleSelectPage.jsx` | 親/子の役割選択 |
| `src/pages/LandingPage.jsx` | 初回訪問ランディング |
| `src/pages/DemoPage.jsx` | デモ体験 — `handleShareDemo` dead code残存 |
| `src/pages/FacilitiesPage.jsx` | 介護施設向けページ — animation内テキスト日本語+日付ハードコード |
| `src/pages/WelcomePage.jsx` | ウェルカム画面 — ⚠️ `lang` prop を受け取らず全テキスト日本語固定 |
| `src/pages/InvitePage.jsx` | 招待ページ — ⚠️ `lang` を `'ja'` にハードコード、props無視 |

### フロントエンド — Components
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | `/pair/:slug` の Outlet ラッパー、slug→pairId解決 |
...
| `src/components/FamilyInsightCard.jsx` | AI家族インサイトカード — ⚠️ cacheKeyがlang/pairId未含 |
...
| `src/components/AdminAuth.jsx` | ⚠️ dead code — どのページからもimportされていない |
| `src/components/OneYearAgoBanner.jsx` | ⚠️ dead code — どのページからもimportされていない |
...
| `src/lib/unreadStateCore.js` | 未読状態コアロジック (verify:unread CIで検証) |
| `src/lib/listenedTracking.js` | 音声再生済みトラッキング |
| `src/lib/indexedDB.js` | IndexedDB キャッシュ管理 |
| `src/lib/invite.js` | 招待リンク生成・解決 |
| `src/lib/inviteShare.js` | 招待シェア機能 |
| `src/lib/fcm.js` | FCMプッシュ通知トークン登録 |
| `src/lib/pairSlug.js` | slug生成・変換ユーティリティ |
| `src/lib/dateFormat.js` | 日付フォーマット (NYタイムゾーン基準) |
| `src/lib/demoPhotos.js` | デモ用写真データ — ⚠️ date labelsが全言語で日本語固定 |
| `src/lib/deployHealthCheck.js` | デプロイヘルスチェック |
| `src/lib/holidayBanner.js` | 祝日バナー |
| `src/lib/shareTargets.js` | シェアターゲット設定 |
...
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history — ⚠️ L816/L1076 `merge:true`残存, PATCHのpairId検証順序誤り |
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST — ⚠️ L536 `merge:true`残存, PAIR-DEMOTESTハードコード比較 |
| `api/album.js` | アルバム全日写真取得 |
| `api/invite.js` | ペア発行(create-numbered), スラグ解決(resolve), pair_users membership claim |
| `api/streak.js` | 連続記録ストリーク — ⚠️ PAIR-DEMOTEST書き込みブロック未実装 |
| `api/daily-theme.js` | AI話題生成 — ⚠️ verifyIdToken未実装、認証なしで全ペアデータアクセス可能 (Critical) |
| `api/family-insight.js` | AI家族インサイト生成 — ⚠️ pairId無効時HTTP 200返却、UTC日付計算 |
| `api/journal-analysis.js` | 写真分析 — ⚠️ L120/L176 `merge:true`残存 |
| `api/admin-reset.js` | 管理リセット — ⚠️ L151 `merge:true`残存 |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | isPairAllowed(), DEMO_PAIR_IDS bypass, READ_ONLY_PAIR_IDS |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase Admin SDK 初期化ヘルパー |
| `api/_disabled/` | 無効化済みAPI群 (analyze.js, analysis-comment.js 等) — フロントから参照残存 |
...
pair_users/{pairId}/members/{uid}                → { role, claimedAt } ← Phase 1 pair-membership
...
- BrowserRouter形式（/pair/:slug/album 等）が全リンクで使われてるか確認（HashRouter `/#/` 記法は廃止済み）
