...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → LandingPage | hum_last_slug有→/pair/:slug リダイレクト, 無→LandingPage |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | ウェルカム画面 |
| `/pair/:slug` | PairWorld (Outlet) | ペア世界のルート (pairId解決) |
...
| `src/pages/WelcomePage.jsx` | ウェルカム画面 (lang prop 受取り未実装 — Medium既知問題) |
| `src/pages/InvitePage.jsx` | 招待ページ (lang='ja' ハードコード — Medium既知問題) |
| `src/components/PairWorld.jsx` | /pair/:slug のネストレイアウト (pairId解決 + Outlet) |
...
| `src/lib/pairMembership.js` | pair membership 管理 (isPairMember, claimMembership) |
| `src/lib/journal.js` | 写真アップロード, fetchTodayJournalMeta, fetchAlbum |
| `src/lib/firebase.js` | Firebase初期化, getIdTokenForApi (匿名認証, self-heal付き) |
...
| `api/daily-theme.js` | AI話題生成 (⚠️ 認証なし — Critical既知問題) |
...
pair_users/{pairId}/members/{uid} → { uid, role, joinedAt }
...
- BrowserRouter形式（/pair/:slug/...）が全リンクで使われてるか確認（HashRouter /#/ 形式の残骸がないか）
