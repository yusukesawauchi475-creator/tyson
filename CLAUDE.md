...
| `/welcome` | WelcomePage | 新規ユーザーオンボーディング ⚠️ lang prop 未受取・全文字列日本語ハードコード |
| `/pair/:slug` | PairWorld | ペアワールド（ネスト） |
| `/pair/:slug/` | RootRoute | 親/子ホーム画面 (role振り分け) |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待ページ ⚠️ lang='ja'固定 |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter ルーティング, PairWorld, AppRoutes |
| `src/pages/HomePage.jsx` | 親のホーム画面 (緑/ピンク/紫の3カード) ⚠️ 無効化済みAPIへのfetch残存 |
| `src/pages/PairDailyPage.jsx` | 子のホーム画面 ⚠️ 無効化済みAPIへのfetch残存・BottomNavインラインスタイル |
...
| `src/pages/WelcomePage.jsx` | 新規ユーザーオンボーディング ⚠️ lang prop 未受取・i18n未対応 |
| `src/pages/InvitePage.jsx` | 招待ページ ⚠️ lang='ja'固定 |
...
| `src/lib/journal.js` | 写真アップロード, fetchTodayJournalMeta, fetchAlbum ⚠️ L69-71デッドコード |
| `src/lib/firebase.js` | Firebase初期化, getIdTokenForApi (匿名認証) |
| `src/lib/i18n.js` | 日英翻訳 |
| `src/lib/pairMembership.js` | ペアメンバーシップ管理 |
| `src/lib/pairSlug.js` | スラグ解決ユーティリティ |
| `src/lib/dateFormat.js` | NY タイムゾーン日付フォーマット |
| `src/lib/demoPhotos.js` | デモ用静的写真データ |
| `src/lib/fcm.js` | FCM プッシュ通知 ⚠️ registerForPush未使用・setDocにmerge:true |
| `src/lib/indexedDB.js` | IndexedDB キャッシュ層 |
...
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history ⚠️ L833,L1110 merge:true |
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST ⚠️ L553 merge:true |
| `api/album.js` | アルバム全日写真取得 ⚠️ isPairAllowed/initFirebaseAdmin 順序依存 |
| `api/invite.js` | ペア発行(create-numbered), スラグ解決(resolve) |
| `api/streak.js` | 連続記録ストリーク ⚠️ PAIR-DEMOTEST書き込みブロック欠落 |
| `api/daily-theme.js` | AI話題生成 (Firebase Auth + pairMembership 検証済み) |
| `api/family-insight.js` | ファミリーインサイト生成 ⚠️ L64 UTC日付でNY dateKeyをクエリ |
| `api/journal-analysis.js` | 写真分析 ⚠️ Firebase Auth なし・ADMIN_PASSWORD のみ |
| `api/admin-reset.js` | 管理リセット ⚠️ PAIR-DEMOTEST書き込みブロック欠落 |
| `api/admin-restore.js` | 管理復元 ⚠️ PAIR-DEMOTEST書き込みブロック欠落 |
| `api/admin-pairs.js` | ペアダッシュボード ⚠️ UTC日付でNY dateKeyをクエリ |
| `api/lib/pair-access.js` | isPairAllowed, isTysonOnlyBlocked 共通ロジック |
