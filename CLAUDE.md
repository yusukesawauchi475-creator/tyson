...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → RootRoute | ランディング or ホーム（role振り分け） |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | ウェルカム/招待受諾ページ |
| `/pair/:slug` | PairWorld (outlet) → RootRoute | ペア世界 (role振り分け) |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待ページ |

## ペアの仕組み
- 公開URL: `humfamily.com/pair/{6文字スラグ}` (例: /pair/ulf1q6)
- Vercel rewrite → `/index.html`（BrowserRouter が `/pair/:slug` を処理）
- `PairWorld.jsx` がスラグから pairId を解決し Outlet context に注入
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けページ (animation, contact form) |
| `src/pages/InvitePage.jsx` | 招待リンク発行・共有 (/pair/:slug/invite) |
| `src/pages/WelcomePage.jsx` | ウェルカム/招待受諾 (/welcome) |
| `src/components/PairWorld.jsx` | ペア世界ラッパー (slug→pairId解決, auth, Outlet) |
...
| `src/components/FamilyInsightCard.jsx` | AIファミリーインサイト表示 |
...
| `src/lib/pairMembership.js` | isPairAllowed, クレーム検証 (Phase 1 pair membership) |
| `src/lib/pairSlug.js` | スラグ生成・解決ユーティリティ |
...
| `api/family-insight.js` | AIファミリーインサイト生成 |
...
| ファイル | 役割 |
|---------|------|
| `vercel.json` | リライト (全パス→index.html), キャッシュヘッダー |
...
### 5. UIの一貫性
- BottomNavがHomePage/AlbumPage/PairDailyPageに存在するか確認 (PairDailyPageは未対応 #62)
- BrowserRouter形式（/path）が全リンクで使われてるか確認（HashRouter `/#/` は廃止）
