...
## ルーティング (BrowserRouter — Phase 1 以降)
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → RootRoute | ランディング or ホーム |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | ペア招待受け入れ完了画面 |
| `/pair/:slug` | PairWorld (layout) | ペア世界のラッパー (membership claim) |
| `/pair/:slug/` (index) | RootRoute | ペアホーム (親/子role振り分け) |
...
| `src/pages/WelcomePage.jsx` | ペア招待受け入れ完了・ペア発行中画面 |
| `src/pages/InvitePage.jsx` | 招待リンク共有 (SMS/LINE/Instagram/Messenger) |
| `src/pages/FacilitiesPage.jsx` | 介護施設向けランディング + アニメーション |
| `src/components/PairWorld.jsx` | `/pair/:slug` layout — slug解決・membership claim・outlet提供 |
...
| `api/daily-theme.js` | AI話題生成 (⚠️ 認証なし — issue #45) |
| `api/family-insight.js` | AI家族インサイト (週次コメント生成) |
| `api/journal-analysis.js` | 写真AI分析 |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | isPairAllowed() — Phase 1 pair-membership canonical check |
...
| `docs/legal/terms-of-use.ja.md` | 利用規約 (日本語) |
| `docs/legal/lawyer-review-checklist.ja.md` | 弁護士レビューチェックリスト |
| `public/privacy.html` | プライバシーポリシー HTML (本番公開) |
| `public/terms.html` | 利用規約 HTML (本番公開) |

## Firestore 構造
```
pair_numbers/{slug}        → { pairId, memo, createdAt }
pairs/{PAIR-XXXXXX}        → { pairId, number(slug), memo, createdAt }
pair_users/{pairId}/members/{uid}   → { uid, claimedAt }  ← Phase 1 pair-membership
...
- **PAIR-DEMOTEST**: デモ用。`READ_ONLY_PAIR_IDS` で書き込みブロック + `DEMO_PAIR_IDS` (pair-access.js) で membership bypass → `isPairAllowed()` が常に true を返すため streak.js 等 READ_ONLY チェック未実装 API での書き込みは別途ブロックが必要
...
- **Phase 1 pair-membership (2026-06-03完了)**: `api/lib/pair-access.js` の `isPairAllowed()` で UID がペアメンバーか確認。`/pair/:slug/` アクセス時に `PairWorld.jsx` が自動 claim。`daily-theme.js` のみ未実装 (issue #45)
...
- BrowserRouter形式（/pair/:slug/album 等）が全リンクで使われてるか確認（`/#/` 形式は旧HashRouter、不要）
