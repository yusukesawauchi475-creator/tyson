...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → RootRoute | pairIdなし→ランディング |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | 新規ユーザーオンボーディング |
| `/pair/:slug` | PairWorld (layout) | ペアコンテキスト |
| `/pair/:slug` (index) | RootRoute | 役割振り分け→HomePage/PairDailyPage |
...
| `src/pages/InvitePage.jsx` | 招待フロー |
...
| `api/journal-analysis.js` | ジャーナル分析 (注: Firebase Auth なし、要修正) |
...
- BrowserRouter形式（/pair/:slug/path）が全リンクで使われてるか確認（HashRouter /#/ 形式残存がないか）
