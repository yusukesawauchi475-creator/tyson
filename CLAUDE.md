...
| `/pair/:slug` | PairWorld → RootRoute | ペアホーム (役割振り分け) |
...
| `src/pages/WelcomePage.jsx` | ウェルカム/オンボーディング画面 |
| `src/pages/FacilitiesPage.jsx` | 介護施設向けランディング |
| `src/pages/InvitePage.jsx` | 招待フロー画面 |
| `src/components/PairWorld.jsx` | `/pair/:slug` レイアウト (slug → pairId 解決, Outlet) |
...
| `api/family-insight.js` | 家族インサイトコメント生成 (OpenAI) |
| `api/journal-analysis.js` | 写真OCR + AI分析 (OpenAI Vision, admin only) |
