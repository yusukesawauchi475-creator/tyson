...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → RootRoute | 最終slug記憶あり→/pair/:slug, 無→LandingPage |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 施設向けランディング |
| `/welcome` | WelcomePage | DEMO CTA経由 新pair発行 |
| `/pair/:slug` | PairWorld (Outlet) | slug解決・メンバーシップ確保・Outlet |
| `/pair/:slug/` | RootRoute (index) | role振り分け → HomePage / PairDailyPage |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム (カレンダー含む) |
| `/pair/:slug/invite` | InvitePage | 招待リンク生成・共有 |
...
| `src/pages/FacilitiesPage.jsx` | 施設向けランディング (デモアニメーション) |
| `src/pages/WelcomePage.jsx` | DEMO CTA経由 新pair自動発行・招待リンク表示 |
| `src/pages/InvitePage.jsx` | 招待リンク生成・LINE/Web Share |
...
| `src/components/PairWorld.jsx` | slug解決・メンバーシップ確保・OutletContext提供 |
...
| `src/lib/pairDaily.js` | getDateKeyNY, getUserRole, markSeen, uploadAudio, fetchAudio, fetchVoiceMonth 等 |
...
| `src/lib/pairMembership.js` | ensureAuthAndMembership (auth+pair_users確保) |
| `src/lib/i18n.js` | 日英翻訳 |
| `src/lib/invite.js` | buildInviteUrl (招待URL生成) |
| `src/lib/inviteShare.js` | buildInviteMessage (招待メッセージ生成) |
| `src/lib/pairSlug.js` | slug関連ユーティリティ |
| `src/lib/shareTargets.js` | SNSシェア先定義 |
| `src/lib/fcm.js` | FCMプッシュ通知登録 (registerForPush — 未使用) |
| `src/lib/listenedTracking.js` | 聴取済みトラッキング |
| `src/lib/unreadState.js` | 未読状態管理 |
| `src/lib/unreadStateCore.js` | 未読状態コアロジック |
| `src/lib/voiceRole.js` | 音声役割判定 (getEffectiveRole 等) |
...
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen/correct) + voice-history + voice-month + role-history-record |
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST (1日3枚制限) |
...
| `api/family-insight.js` | 家族インサイト生成 (AI, UTC日付バグあり) |
| `api/journal-analysis.js` | ジャーナル画像OCR + AI分析 (X-Admin-Password認証のみ) |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | isPairAllowed, isTysonOnlyBlocked (共通アクセス制御) |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase SA JSON パース (Vercel env 対応) |
