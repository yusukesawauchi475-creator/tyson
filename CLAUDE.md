...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → RootRoute | pairId有→ランディング, 無→ランディング |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向け B2B ランディング |
| `/welcome` | WelcomePage | 初回ウェルカム画面 |
| `/pair/:slug` | PairWorld (Outlet) | ペアワールド（slug でペア識別） |
| `/pair/:slug` (index) | RootRoute | 親/子ホーム画面 |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待リンク共有 |

※ vercel.json の SPA rewrite (`/(.*) → /index.html`) により全パスが index.html に向く
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter, AppRoutes, RootRoute (role振り分け), PwaInstallBanner |
| `src/pages/HomePage.jsx` | 親のホーム画面 (緑/ピンク/紫の3カード) ⚠️ 無効API参照残存 |
| `src/pages/PairDailyPage.jsx` | 子のホーム画面 ⚠️ 無効API参照残存, today dead code |
| `src/pages/AlbumPage.jsx` | アルバム (写真タブ/声タブ, ライトボックス, badge) ⚠️ demo日付JP hardcode |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向け B2B ランディング ⚠️ VITE_FORMSPREE_ENDPOINT 未設定 |
| `src/pages/WelcomePage.jsx` | 初回ウェルカム画面 ⚠️ lang prop 無視 |
| `src/pages/InvitePage.jsx` | 招待リンク共有 ⚠️ lang='ja' hardcode |

### Components
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug のコンテキスト提供 (pairId解決, Outlet) |
...
| `src/components/InviteModal.jsx` | シェアモーダル (messenger/clipboard) ⚠️ inviteUrl prop 無視 |
| `src/components/DemoModal.jsx` | デモ用CTA モーダル |
| `src/components/AlbumCalendar.jsx` | アルバムカレンダービュー |
| `src/components/LanguageSwitch.jsx` | 言語切替 (ja/en/es + 国旗) |
| `src/components/Visualizer.jsx` | 録音中 RMS 波形オーバーレイ |
| `src/components/UploadErrorModal.jsx` | アップロードエラー表示モーダル |
| `src/components/RoleBadge.jsx` | 親/子ロールバッジ |
| `src/components/OneYearAgoBanner.jsx` | 1年前の今日バナー ⚠️ UTC日付計算 / どこにもimportなし |
| `src/components/FamilyInsightCard.jsx` | 家族インサイトカード ⚠️ langなしcacheキー / どこにもimportなし |
| `src/components/AdminAuth.jsx` | 管理画面認証 ⚠️ dead code (importなし) |
...
| `src/lib/i18n.js` | 日/英/西3言語翻訳 |
| `src/lib/inviteShare.js` | messenger別URL生成 (WhatsApp/LINE/Messenger等) |
| `src/lib/invite.js` | 招待リンク生成ユーティリティ |
| `src/lib/listenedTracking.js` | 未聴フラグ管理 ⚠️ getAnyPartnerUnlistenedFlagServerSeen は exportのみ |
| `src/lib/demoPhotos.js` | デモ写真データ ⚠️ date labels JP固定 |
| `src/lib/holidayBanner.js` | 祝日countdown banner ロジック |
| `src/lib/dateFormat.js` | NY時間ベース日付フォーマット |
| `src/lib/pairSlug.js` | slug生成/解決ユーティリティ |
| `src/lib/uiCopy.js` | UI文言定数 |
| `src/lib/tysonThemes.js` | テーマ設定 |
| `src/lib/voiceRole.js` | 音声ロール判定 |
| `src/lib/fcm.js` | FCMプッシュ通知クライアント |
| `src/lib/indexedDB.js` | IndexedDBキャッシュ |
| `src/lib/shareTargets.js` | シェアターゲット定義 |
| `src/lib/useAudioLevel.js` | AudioLevel React hook |
...
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history ⚠️ merge:true L811,1071 |
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST ⚠️ merge:true L541 |
| `api/album.js` | アルバム全日写真取得 |
| `api/invite.js` | ペア発行(create-numbered), スラグ解ععlug解決(resolve) |
| `api/streak.js` | 連続記録ストリーク ⚠️ POST が PAIR-DEMOTEST 書き込みブロックなし |
| `api/daily-theme.js` | AI話題生成 ⚠️ verifyIdToken なし (Critical) |
| `api/family-insight.js` | 家族インサイトAIコメント ⚠️ UTC日付計算, !pairId時200返却 |
| `api/journal-analysis.js` | 写真OCR分析 (管理者専用) ⚠️ merge:true L120,176 |
| `api/admin-reset.js` | 管理リセット ⚠️ merge:true L156 |
| `api/admin-restore.js` | 管理復元 ⚠️ merge:true L150 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | ペアアクセス制御 ⚠️ TYSON_ZH90_ALLOWED_UIDS hardcode |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase Admin初期化ヘルパー |
| `api/_disabled/` | 無効化済みAPI群 (analyze, analysis-comment 等) |
...
### 2. データ整合性
- 全 API (pair-media.js, journal.js, admin-reset.js, admin-restore.js, journal-analysis.js) の set() が merge:true なしか確認
  - 既知: pair-media.js L811,1071 / journal.js L541 / admin-reset.js L156 / admin-restore.js L150 / journal-analysis.js L120,176 に残存
...
- BrowserRouter形式（/pair/:slug/album 等）が全リンクで使われてるか確認（/#/ 形式は廃止済み）
