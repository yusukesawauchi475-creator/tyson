...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → RootRoute | localStorage slug有→/pair/:slug復元, 無→LandingPage |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | 初回訪問ランディング |
| `/facilities` | FacilitiesPage | 介護施設向けランディング |
| `/welcome` | WelcomePage | ウェルカムページ (lang prop 未実装・要Phase3) |
| `/pair/:slug` | PairWorld (Outlet) | slug→pairId解決コンテキスト、メンバーシップ確認 |
| `/pair/:slug/` | RootRoute | 親/子ホーム画面 (role振り分け) |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待ページ (lang='ja'固定・要Phase3) |

## ペアの仕組み
- 公開URL: `humfamily.com/pair/{スラグ}` (例: /pair/ulf1q6)
- Vercel rewrite → SPA → PairWorld コンポーネントが client-side で Firestore `pair_numbers/{slug}` を読み込み pairId に解決
- `ensureAuthAndMembership()` で匿名認証 + メンバーシップ確認後、Outlet context として子コンポーネントへ `{ pairId, slug, lang }` を渡す
- Firestore: `pair_numbers/{slug}` → `pairId` (例: PAIR-H58HTP)
- 内部ID `PAIR-XXXXXX` は URL に露出しない
- PWA install後の `/` アクセス: localStorage `hum_last_slug` から `/pair/:slug` に自動復元
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けランディング (静的日付hardcode・低優先修正候補) |
| `src/pages/InvitePage.jsx` | 招待ページ (lang='ja'固定、Phase3で多言語対応予定) |
| `src/pages/WelcomePage.jsx` | ウェルカムページ (lang prop 無視・既知バグ、Phase3で修正予定) |

### フロントエンド — コンポーネント
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug のコンテキストプロバイダ (slug→pairId解決, 認証) |
| `src/components/DailyPromptCard.jsx` | 今日の話題pill (AI話題 + 別の話題ボタン) |
| `src/components/FamilyInsightCard.jsx` | AI家族分析カード |
...
| `src/components/OneYearAgoBanner.jsx` | 1年前バナー (未使用dead code・削除候補) |
| `src/components/AdminAuth.jsx` | 管理認証 (未使用dead code・削除候補) |
...
| `src/lib/invite.js` / `inviteShare.js` | 招待リンク生成・共有 |
| `src/lib/shareTargets.js` | SNS共有ターゲット |
...
| `api/family-insight.js` | AI家族分析コメント生成 |
| `api/journal-analysis.js` | 日記分析 (AdminPage から呼び出し) |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/_disabled/` | 無効化済み関数 (analyze, analysis-comment, notify 等) |
