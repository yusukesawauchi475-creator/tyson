...
| `/pair/:slug` | PairWorld | ペアの世界（ネストルート親） |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter ルーティング, RootRoute (role振り分け) |
| `src/pages/HomePage.jsx` | 親のホーム画面 (緑/ピンク/紫の3カード) ⚠️ disabled API fetch残存 |
| `src/pages/PairDailyPage.jsx` | 子のホーム画面 ⚠️ disabled API fetch残存 |
...
| `src/pages/FacilitiesPage.jsx` | 施設向けランディングページ |
| `src/pages/WelcomePage.jsx` | 新規ペア作成後オンボーディング ⚠️ lang prop 未受取・全文字列日本語固定 |
| `src/pages/InvitePage.jsx` | 招待リンク共有 ⚠️ lang='ja' 固定 |
...
| `src/components/PairWorld.jsx` | /pair/:slug ネストルート親コンポーネント |
...
| `src/components/Visualizer.jsx` | 録音波形ビジュアライザ |
| `src/lib/pairDaily.js` | getPairId, getUserRole, markSeen, uploadAudio, fetchAudio 等 |
| `src/lib/journal.js` | 写真アップロード, fetchTodayJournalMeta, fetchAlbum ⚠️ L69-71 デッドコード |
| `src/lib/firebase.js` | Firebase初期化, getIdTokenForApi (匿名認証) |
| `src/lib/i18n.js` | 日英翻訳 |
| `src/lib/pairMembership.js` | ペアメンバーシップ検証 (ensureAuthAndMembership) |
| `src/lib/pairSlug.js` | スラグ解決ユーティリティ |
| `src/lib/fcm.js` | FCM push通知 (親専用) |
...
| `src/lib/inviteShare.js` | 招待リンク共有ユーティリティ |
| `src/lib/listenedTracking.js` | 音声再生済み追跡 (localStorage) |
| `src/lib/shareTargets.js` | シェアターゲット定義 |
| `src/lib/tysonThemes.js` | テーマ定義 |
| `src/lib/uiCopy.js` | UIコピーテキスト管理 |
| `src/lib/unreadState.js` | 未読状態管理 |
| `src/lib/unreadStateCore.js` | 未読状態コアロジック |
| `src/lib/useAudioLevel.js` | 音声レベルフック |
...
| `api/album.js` | アルバム全日写真取得 ⚠️ isPairAllowed/initFirebaseAdmin順序依存 |
...
| `api/admin-pairs.js` | ペアダッシュボード ⚠️ UTC日付でNY dateKey検索 |
| `api/family-insight.js` | ファミリーインサイト生成 ⚠️ UTC日付でNY dateKey検索 |
| `api/journal-analysis.js` | ジャーナル分析 ⚠️ Firebase Auth/pairId検証なし |
| `api/lib/pair-access.js` | isPairAllowed, isTysonOnlyBlocked |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase SA解析 |
| `api/_disabled/` | 無効化済みAPI (analyze.js, analysis-comment.js等) |
