...
- **i18n**: 3言語対応 (ja / en / es), `src/lib/i18n.js`
- **テスト**: Vitest (`tests/unreadState.test.mjs`)
- **CI**: GitHub Actions (`.github/workflows/ci.yml`) — `npm run verify:unread` のみ (lint 未カバー)

## ルーティング (BrowserRouter)
vercel.json で `/*` → `index.html` SPA rewrite 済み。
...
| `/welcome` | WelcomePage | ウェルカムページ |
| `/pair/:slug` | PairWorld | pair ルートコンテナ (slug→pairId 解決) |
| `/pair/:slug` (index) | RootRoute | 親/子のホーム画面 |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待ページ ⚠️ lang='ja' hardcode |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter ルーティング, AppRoutes, RootOrLanding |
| `src/pages/HomePage.jsx` | 親のホーム画面 (緑/ピンク/紫の3カード) ⚠️ /api/analyze 参照残存 |
| `src/pages/PairDailyPage.jsx` | 子のホーム画面 ⚠️ /api/analyze 参照残存, today state dead |
| `src/pages/AlbumPage.jsx` | アルバム (写真タブ/声タブ, ライトボックス) ⚠️ demo日付ラベル日本語 |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けLP ⚠️ animation内テキスト日本語固定・日付hardcode |
| `src/pages/WelcomePage.jsx` | ウェルカムページ ⚠️ lang prop 無視 (全文字列日本語hardcode) |
| `src/pages/InvitePage.jsx` | 招待ページ ⚠️ lang='ja' hardcode (i18n 三項 dead code) |

### Components
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | pair ルートコンテナ (slug→pairId 解決, Outlet) |
...
| `src/components/InviteModal.jsx` | 招待モーダル (messenger/LINE振り分け) |
| `src/components/DemoModal.jsx` | デモCTAモーダル |
| `src/components/AlbumCalendar.jsx` | アルバムカレンダービュー |
| `src/components/LanguageSwitch.jsx` | ja/en/es 言語切替 |
| `src/components/Visualizer.jsx` | 録音中音声波形visualizer |
| `src/components/UploadErrorModal.jsx` | アップロードエラーモーダル |
| `src/components/RoleBadge.jsx` | 親/子ロールバッジ |
| `src/components/OneYearAgoBanner.jsx` | 1年前の記録バナー ⚠️ dead code (どこにも import なし) |
| `src/components/FamilyInsightCard.jsx` | AI家族インサイトカード ⚠️ dead code + cacheKey が lang/pairId 無視 |
| `src/components/AdminAuth.jsx` | 管理画面認証 ⚠️ dead code (どこにも import なし) |
...
| `src/lib/i18n.js` | ja/en/es 翻訳辞書 |
| `src/lib/unreadState.js` | 未読状態管理 (getPartnerUnreadState 等) |
| `src/lib/unreadStateCore.js` | 未読コアロジック (純粋計算) |
| `src/lib/voiceRole.js` | 音声ロール判定ユーティリティ |
| `src/lib/listenedTracking.js` | isVoiceListened / markVoiceListened (IndexedDB) |
| `src/lib/demoPhotos.js` | デモ用写真データ ⚠️ date labels 全言語で日本語固定 |
| `src/lib/dateFormat.js` | 日付フォーマットユーティリティ (NY timezone) |
| `src/lib/pairSlug.js` | slug生成・解決ユーティリティ |
| `src/lib/uiCopy.js` | UI文字列コピーユーティリティ |
| `src/lib/fcm.js` | Firebase Cloud Messaging (FCM) クライアント |
| `src/lib/shareTargets.js` | 共有ターゲット (messenger/LINE/etc.) |
| `src/lib/indexedDB.js` | IndexedDB ラッパー |
| `src/lib/tysonThemes.js` | テーマ定義 (Caribbean等) |
| `src/lib/invite.js` | 招待リンク生成 |
| `src/lib/holidayBanner.js` | 国別祝日バナーデータ |
| `src/lib/deployHealthCheck.js` | デプロイヘルスチェック |
| `src/lib/inviteShare.js` | 招待シェアユーティリティ |
| `src/lib/useAudioLevel.js` | 録音音量レベル取得 hook |
...
| `api/streak.js` | 連続記録ストリーク ⚠️ POST が PAIR-DEMOTEST 書き込みをブロックしない |
| `api/daily-theme.js` | AI話題生成 ⚠️ verifyIdToken なし (認証バイパス可能) |
| `api/family-insight.js` | AI家族インサイト ⚠️ UTC日付計算, !pairId で200返却 |
| `api/journal-analysis.js` | 写真分析 ⚠️ L120/176 merge:true |
...
| ファイル | 役割 |
|---------|------|
| `vercel.json` | リダイレクト (/pair/:slug), SPA rewrite (/* → index.html), キャッシュヘッダー |
...
| `.github/workflows/ci.yml` | CI (npm run verify:unread のみ) ⚠️ lint未カバー |
| `tests/unreadState.test.mjs` | unreadState 単体テスト (Vitest) |
...
### 2. データ整合性
- 全 API の set() が merge:true なしか確認（既知 bug 箇所: pair-media.js L811/1071, journal.js L541, admin-reset.js L156, admin-restore.js L150, journal-analysis.js L120/176）
...
- BrowserRouter形式（/pair/:slug）が全リンクで使われてるか確認（旧 HashRouter /#/ 形式が混入していないか）
