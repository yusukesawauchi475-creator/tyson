...
- **認証**: Firebase Anonymous Auth (pair membership は `pair_users/{pairId}/members/{uid}` で管理)
- **デプロイ**: Vercel Pro（mainブランチ push で自動デプロイ）
- **フォント**: Nunito (Google Fonts, 700/800)
- **言語**: ja / en / es (`?lang=` URL param または localStorage `hum_lang`)

## ルーティング (BrowserRouter)
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding → LandingPage | 前回slug有→/pair/:slug, 無→LandingPage |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | 初回ウェルカム |
| `/pair/:slug` | PairWorld (outlet) | pair コンテキスト wrapper |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | ルーティング, RootOrLanding, AppRoutes, RootRoute (role振り分け) |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けページ (/facilities) |
| `src/pages/WelcomePage.jsx` | 初回ウェルカム画面 (/welcome) |
| `src/pages/InvitePage.jsx` | 招待ページ (/pair/:slug/invite) |

### フロントエンド — Components
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug の outlet wrapper (pairId解決, auth管理) |
...
| `src/components/AdminAuth.jsx` | 管理認証 (dead code — どこからも import されていない) |
| `src/components/OneYearAgoBanner.jsx` | 1年前バナー (dead code — どこからも import されていない) |
...
| `src/lib/pairMembership.js` | ensureAuthAndMembership() — auth self-heal + membership claim |
| `src/lib/i18n.js` | 日英西翻訳 (ja/en/es) |
| `src/lib/unreadState.js` | 未読状態管理 |
| `src/lib/unreadStateCore.js` | 未読状態コアロジック (pure compute) |
| `src/lib/fcm.js` | FCM プッシュ通知クライアント |
| `src/lib/indexedDB.js` | オフラインキャッシュ (IndexedDB) |
| `src/lib/invite.js` | 招待URL生成・解決 |
| `src/lib/inviteShare.js` | 招待シェア |
| `src/lib/voiceRole.js` | 声ロール管理 |
| `src/lib/listenedTracking.js` | 再生済みトラッキング |
| `src/lib/pairSlug.js` | スラグユーティリティ |
| `src/lib/uiCopy.js` | UIコピーテキスト |
| `src/lib/shareTargets.js` | シェアターゲット定義 |
| `src/lib/tysonThemes.js` | 日替わりテーマ |
| `src/lib/useAudioLevel.js` | 音量レベルフック |
...
| `api/daily-theme.js` | AI話題生成 (要注意: 認証なし — issue #56) |
| `api/family-insight.js` | AI家族インサイトコメント生成 |
| `api/journal-analysis.js` | ジャーナル分析 |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | ペアアクセス権限チェック共通ライブラリ |
| `api/lib/firebase-admin.js` | Firebase Admin 初期化共通ライブラリ |
...
pair_users/{pairId}/members/{uid}   → { role, claimedAt } ← pair membership (Phase 1)
