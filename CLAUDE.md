...
| `src/components/PairWorld.jsx` | ペアワールドコンテキスト (pair/:slug outlet, auth+membership管理) |
...
| `src/components/DemoModal.jsx` | デモ操作時モーダル (書き込みブロック通知) |
...
| `src/components/Visualizer.jsx` | 録音再生時の音声波形ビジュアライザー |
...
| `src/lib/pairMembership.js` | ensureAuthAndMembership, claimPairMembership |
| `src/lib/pairSlug.js` | generateSlug() Crockford Base32 スラグ生成 |
| `src/lib/unreadState.js` | 未読状態管理 (fetchUnread 等) |
| `src/lib/unreadStateCore.js` | 未読状態コアロジック (pure compute) |
| `src/lib/voiceRole.js` | immutable訂正追記モデルでの声のrole決定 |
| `src/lib/listenedTracking.js` | 既聴トラッキング (isVoiceListened) |
| `src/lib/fcm.js` | FCMプッシュ通知 (token取得・Firestore保存) |
...
| `api/daily-theme.js` | AI話題生成 (未認証: Critical bug) |
| `api/family-insight.js` | 家族インサイト1行コメント生成 (OpenAI) |
| `api/journal-analysis.js` | ジャーナル写真AI分析 (admin-only, POST) |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/` | API共有ヘルパー (pair-access.js 等) |
