...
| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding | ランディング or /pair/:slug へリダイレクト |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | 初回訪問ランディング |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | 新規ユーザーウェルカム |
| `/pair/:slug` | PairWorld (layout) | ペア専用レイアウト (slug → pairId 解決) |
| `/pair/:slug/` (index) | RootRoute | 役割別ホーム (HomePage or PairDailyPage) |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向け紹介ページ (Formspree contact form) |
| `src/pages/InvitePage.jsx` | ペア招待ページ (/pair/:slug/invite) |
| `src/pages/WelcomePage.jsx` | 新規ユーザーウェルカム (lang prop 未実装 — 要修正) |
...
| `src/components/FamilyInsightCard.jsx` | AI家族インサイトカード (lang非対応キャッシュキー — 要修正) |
| `src/components/InviteModal.jsx` | 招待リンクモーダル |
| `src/components/LanguageSwitch.jsx` | 言語切替 (ja/en) |
| `src/components/OneYearAgoBanner.jsx` | 1年前の思い出バナー (dead code — 未使用) |
| `src/components/PairWorld.jsx` | ペアレイアウト wrapper (slug→pairId解決, Outlet) |
...
| `src/components/AdminAuth.jsx` | 管理認証 (dead code — どこからもimportなし) |
...
| `src/lib/pairMembership.js` | ペアメンバーシップ検証 (claim/verify, retry logic) |
| `src/lib/pairSlug.js` | slug ↔ pairId 変換ヘルパー |
| `src/lib/unreadState.js` | 未読状態管理 (UI layer) |
| `src/lib/unreadStateCore.js` | 未読状態コアロジック (pure compute) |
| `src/lib/indexedDB.js` | IndexedDB キャッシュ (オフライン対応) |
| `src/lib/inviteShare.js` | 招待リンク生成・共有 |
| `src/lib/shareTargets.js` | Web Share API ターゲット |
| `src/lib/tysonThemes.js` | テーマ定義 (色・スタイル) |
| `src/lib/uiCopy.js` | UIコピー文字列管理 |
...
| `src/lib/demoPhotos.js` | デモ写真データ (日付ラベルが全言語日本語固定 — 要修正) |
...
| `api/daily-theme.js` | AI話題生成 (⚠️ 認証なし・pairIdバリデーションなし — 要修正) |
| `api/family-insight.js` | AI家族インサイト生成 |
| `api/journal-analysis.js` | 日記分析 API |
...
- BrowserRouter形式（/pair/:slug/album 等）が全リンクで使われてるか確認 (HashRouter /#/ は廃止)
