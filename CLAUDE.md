...
| `/pair/:slug` | PairWorld (Outlet) | ペアワールド (slug で pair resolve) |
| `/pair/:slug/` (index) | RootRoute | ホーム画面 (role で親/子振り分け) |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けランディング (アニメーションデモ) |
| `src/pages/WelcomePage.jsx` | 新規ユーザーオンボーディング |
| `src/pages/InvitePage.jsx` | 招待フロー (/pair/:slug/invite) |

### フロントエンド — Components
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | /pair/:slug Outlet — slug→pairId resolve + auth guard |
| `src/components/DailyPromptCard.jsx` | 今日の話題pill (AI話題 + 別の話題ボタン) |
| `src/components/VoiceLibrary.jsx` | 声の履歴一覧 (アルバム声タブ用) |
| `src/components/AlbumCalendar.jsx` | 月次カレンダー (写真・声有無を俯瞰) |
| `src/components/PwaInstallBanner.jsx` | Android PWAインストールバナー |
| `src/components/WeeklySummary.jsx` | 週次サマリー (日曜のみ) |
| `src/components/RoleBadge.jsx` | 現在role常時表示バッジ (親/子切替トリガ) |
| `src/components/InviteModal.jsx` | 招待モーダル (LINE/メッセージ共有) |
| `src/components/UploadErrorModal.jsx` | 音声・写真アップロードエラーモーダル |
| `src/components/FamilyInsightCard.jsx` | 家族インサイトカード (AI分析) |
...
| `src/components/Visualizer.jsx` | 音声録音中の波形ビジュアライザ |

### フロントエンド — Lib
| ファイル | 役割 |
|---------|------|
| `src/lib/pairDaily.js` | getUserRole, markSeen, uploadAudio, fetchAudio, fetchVoiceMonth 等 |
...
| `src/lib/pairMembership.js` | ensureAuthAndMembership (auth + membership 保証) |
| `src/lib/i18n.js` | 日英西翻訳 (t関数) |
| `src/lib/invite.js` | buildInviteUrl (invite URL生成) |
| `src/lib/inviteShare.js` | buildInviteMessage, getMessengerTargets (共有メッセージ) |
| `src/lib/voiceRole.js` | getEffectiveRole, isCorrected (role補正ロジック) |
| `src/lib/dateFormat.js` | formatTime12hLowerFromHHMM 等 日付フォーマット |
| `src/lib/listenedTracking.js` | isVoiceListened, markVoiceListened (再生済みトラッキング) |
| `src/lib/unreadState.js` | 未読状態管理 |
| `src/lib/unreadStateCore.js` | 未読状態コアロジック |
| `src/lib/pairSlug.js` | slug ユーティリティ |
| `src/lib/fcm.js` | registerForPush (FCMプッシュ通知登録) |
...
| `src/lib/demoPhotos.js` | デモ写真URL定義 |
| `src/lib/deployHealthCheck.js` | デプロイヘルスチェック |
| `src/lib/useAudioLevel.js` | 音声レベル検知 hook |
...
| `api/invite.js` | ペア発行(create-numbered), スラグ解決(resolve), membership claim |
| `api/streak.js` | 連続記録ストリーク |
| `api/daily-theme.js` | AI話題生成 (verifyIdToken + isPairAllowed 認証) |
| `api/family-insight.js` | 家族インサイト生成 (AI分析) |
| `api/journal-analysis.js` | ジャーナル分析 (X-Admin-Password認証のみ) |
| `api/admin-reset.js` | 管理リセット |
| `api/admin-restore.js` | 管理復元 |
| `api/admin-pairs.js` | ペアダッシュボード |
| `api/lib/pair-access.js` | isPairAllowed, DEMO_PAIR_IDS (pair membership check共通) |
