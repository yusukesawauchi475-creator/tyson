...
| `/pair/:slug` | PairWorld (Outlet) | ペアワールド (pairId 解決 + Outlet) |
| `/pair/:slug/` | RootRoute | ペア内ホーム (role振り分け) |
...
| ファイル | 役割 |
|---------|------|
| `src/App.jsx` | BrowserRouter + Routes定義, PairWorld, RootRoute (role振り分け) |
...
| `src/pages/WelcomePage.jsx` | ウェルカム画面 (**注**: lang prop 受け取りなし → i18n未対応) |
| `src/pages/InvitePage.jsx` | 招待ページ (/pair/:slug/invite) |

### コンポーネント (src/components/)
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | pairId解決 + Outlet (BrowserRouter nested route の親) |
...
| `src/components/AdminAuth.jsx` | (**dead code**: src/内にimportなし、削除候補) |
...
| `src/lib/useAudioLevel.js` | 音量レベル取得フック |
| `src/lib/voiceRole.js` | 声ロール定義 |
| `src/index.css` | グローバルCSS (.page, .bottom-nav 等) |

### API (Vercel Serverless)
| ファイル | 役割 |
|---------|------|
| `api/pair-media.js` | 音声 GET/POST/PATCH(markSeen) + voice-history (**注**: L833,L1110 に merge:true バグ) |
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST (**注**: L553 に merge:true バグ) |
| `api/album.js` | アルバム全日写真取得 |
| `api/invite.js` | ペア発行(create-numbered), スラグ解決(resolve) |
| `api/streak.js` | 連続記録ストリーク (**注**: PAIR-DEMOTEST write block 欠落) |
| `api/daily-theme.js` | AI話題生成 (**注**: pairId欠落時でも200 + OpenAI呼び出しが走るバグ) |
| `api/admin-reset.js` | 管理リセット (**注**: PAIR-DEMOTEST write block 欠落) |
| `api/admin-restore.js` | 管理復元 (**注**: PAIR-DEMOTEST write block 欠落) |
| `api/admin-pairs.js` | ペアダッシュボード (**注**: 30日カレンダー日付がUTC固定) |
| `api/family-insight.js` | ファミリーインサイトコメント (Firebase Auth + isPairAllowed 実装済み) |
| `api/journal-analysis.js` | 日記OCR分析 (**Critical**: Firebase Auth なし・任意pairId書き込み可) |
| `api/lib/pair-access.js` | isPairAllowed ヘルパー (全API共通) |
| `api/lib/parseFirebaseServiceAccount.js` | Firebase SA パース |
| `api/_disabled/` | 無効化済みエンドポイント群 (analyze.js, analysis-comment.js 等) |
