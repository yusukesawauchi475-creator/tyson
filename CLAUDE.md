...
vercel.json の SPA rewrite (`/(?!api/)(.*)` → `/index.html`) で全パスが React に渡される。

| パス | コンポーネント | 説明 |
|------|-------------|------|
| `/` | RootOrLanding | localStorage slug あり→ `/pair/:slug` へ, 無→ LandingPage |
| `/admin` | AdminPage | 管理画面 |
| `/demo` | DemoPage | デモ体験 |
| `/landing` | LandingPage | ランディングページ |
| `/facilities` | FacilitiesPage | 介護施設向けページ |
| `/welcome` | WelcomePage | 招待後ウェルカムページ |
| `/pair/:slug` | PairWorld → RootRoute | slug解決後, role振り分け (RoleSelectPage / HomePage / PairDailyPage) |
| `/pair/:slug/album` | AlbumPage | 写真・声アルバム |
| `/pair/:slug/invite` | InvitePage | 招待リンク発行 |
...
| `src/pages/FacilitiesPage.jsx` | 介護施設向けページ (contact form 要 VITE_FORMSPREE_ENDPOINT) |
| `src/pages/WelcomePage.jsx` | 招待後ウェルカム (⚠️ lang prop 無視, 日本語 hardcode) |
| `src/pages/InvitePage.jsx` | 招待リンク発行 (⚠️ lang='ja' hardcode) |

### Components
| ファイル | 役割 |
|---------|------|
| `src/components/PairWorld.jsx` | `/pair/:slug` wrapper, slug→pairId 解決, outlet context 提供 |
...
| `src/components/DemoModal.jsx` | デモ誘導CTA モーダル |
| `src/components/PwaInstallBanner.jsx` | Android PWAインストールバナー |
| `src/components/WeeklySummary.jsx` | 週次サマリー (日曜のみ) |
| `src/components/Visualizer.jsx` | 録音/再生 波形ビジュアライザー |
| `src/components/LanguageSwitch.jsx` | ja/en/es 言語切替 |
| `src/components/RoleBadge.jsx` | 親/子 バッジ表示 |
| `src/components/UploadErrorModal.jsx` | アップロードエラー通知モーダル |
| `src/components/FamilyInsightCard.jsx` | AI家族インサイトカード (⚠️ lang無視のキャッシュキー) |
| `src/components/OneYearAgoBanner.jsx` | 1年前バナー (⚠️ どこにも import されていない) |
| `src/components/AdminAuth.jsx` | 管理認証 (⚠️ dead code — どこにも import されていない) |
...
| `src/lib/listenedTracking.js` | 既聴トラッキング (⚠️ getAnyPartnerUnlistenedFlagServerSeen は未使用) |
| `src/lib/dateFormat.js` | 日付フォーマット (NY timezone) |
| `src/lib/demoPhotos.js` | デモ写真データ (⚠️ 日付ラベル全言語で日本語固定) |
| `src/lib/tysonThemes.js` | Caribbean テーマ定義 |
| `src/lib/holidayBanner.js` | 国別記念日データ |
| `src/lib/invite.js` | 招待リンク生成 |
| `src/lib/inviteShare.js` | 招待共有 (messenger/LINE 等) |
| `src/lib/shareTargets.js` | 共有ターゲット定義 |
| `src/lib/fcm.js` | Firebase Cloud Messaging (プッシュ通知) |
| `src/lib/indexedDB.js` | IndexedDB キャッシュ |
| `src/lib/uiCopy.js` | UI テキスト定数 |
| `src/lib/useAudioLevel.js` | 音声レベル計測 React hook |
...
| `api/journal.js` | 写真 GET(今日→最新日フォールバック)/POST (⚠️ merge:true L541, pairId fallback 'demo' L402) |
| `api/album.js` | アルバム全日写真取得 |
| `api/invite.js` | ペア発行(create-numbered), スラグ解決(resolve) |
| `api/streak.js` | 連続記録ストリーク (⚠️ POST が PAIR-DEMOTEST write block なし) |
| `api/daily-theme.js` | AI話題生成 (⚠️ 認証なし — 任意 pairId でデータ取得可能) |
| `api/family-insight.js` | AI家族インサイト (⚠️ pairId欠落時 HTTP 200, 日付計算UTC) |
| `api/journal-analysis.js` | 日記分析 (⚠️ merge:true L120,176) |
| `api/admin-reset.js` | 管理リセット (⚠️ pairId fallback 'demo' L110, merge:true L156) |
| `api/admin-restore.js` | 管理復元 (⚠️ pairId fallback 'demo' L109, merge:true L150) |
...
### 2. データ整合性
- 全 API の set() で merge:true が使われていないか確認 (既知違反: pair-media.js L811/1071, journal.js L541, journal-analysis.js L120/176, admin-reset.js L156, admin-restore.js L150)
...
- BrowserRouter形式（/pair/:slug/album 等）が全リンクで使われてるか確認（HashRouter形式 /#/ は廃止済み）
